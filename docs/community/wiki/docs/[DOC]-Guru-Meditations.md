...
Similar to `in obj 'word` for objects, `in :fun 'word` was implemented as a fast way to check if word is supported by the function (may be useful before calling `apply`).

Some historical workarounds:
