...
ctx: context [
	...
	transparent: 0.0.0.254
	colors: [aqua beige black blue brick brown coal coffee crimson cyan forest gold 
gray green ivory khaki leaf linen magenta maroon mint navy oldrab olive orange papaya pewter 
pink purple reblue rebolor Red sienna silver sky snow tanned teal transparent violet water 
wheat white yello yellow]
	pallette: [
	...
	make-pallette
	color: black
	select-color: does [view pallette];[view/flags pallette [modal popup]]
	...
	view win: layout/flags/options [
		...
		tp: panel [
			at 0x0 box white 320x220 react [face/size: face/parent/size];draw [line 12x2 12x20]
			sb: rich-text 300x200 "" focus react [face/size: face/parent/size - 20]
			...
			on-dbl-click [
				start: either found: find/reverse at face/text offset-to-caret face event/offset sep [
					2 + (index? found) - index? face/text
				][
					1
				]
				end: either found: find next at face/text start sep [
					2 + (index? found) - index? face/text
				][
					1 + length? face/text
				]
				...
				either event/ctrl? [
					switch event/key [ 
						...
						left  [
							either event/shift? [
								either end > start  [
									end: either found: find/reverse back at face/text face/data/1/x + face/data/1/y sep [
										2 + (index? found) - index? face/text
									][
										1
									]
									...
									set-caret/diff face end: either found: find/reverse back at face/text face/data/1/x sep [
										2 + (index? found) - index? face/text
									][
										1
								...
								set-caret face either found: find/reverse back at face/text face/data/1/x sep [
									2 + (index? found) - index? face/text
								][
									1
								]
						...
						right [
							either event/shift? [
								either end < start [
									end: either found: find next at face/text face/data/1/x sep [
										2 + (index? found) - index? face/text
									][
										1 + length? face/text
									]
									...
									set-caret/diff face face/data/1/x 
										(end: either found: find next at face/text face/data/1/x + face/data/1/y sep [
											2 + (index? found) - index? face/text
										][
											1 + length? face/text
										]) - face/data/1/x
								...
								set-caret face either found: find next at face/text face/data/1/x sep [
									2 + (index? found) - index? face/text
								][
									1 + length? face/text
								]
						...
						#"^~" [
							remove/part at face/text end: either found: find/reverse back at face/text face/data/1/x sep [
								2 + (index? found) - index? face/text
							][
								1
							] (face/data/1/x - end)
						...
						home [
							if 1 < index? face/text [
								start: start + index? face/text 
								face/text: head face/text 
								clear itext
							] 
							either event/shift? [
						...
						down [
							...
							either event/shift? [
								...
								either found [
									set-caret face 2 + (index? found) - index? face/text
								][
									set-caret face 1 + length? face/text
								]
							]
			...
			at 10x10 crt: box glass 300x200 rate 3 react [face/size: face/parent/size - 20]
		...
		actors: object [
			on-menu: function [face event][
				if file: request-file/title "Open a file" [
					sb/text: read file
				]
			]
			on-resizing: func [face event][
				tp/size: face/size - tp/offset - 10
			]
		]
	]
]
