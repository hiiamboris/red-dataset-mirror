...
a+v: function [
	vfile [file!]
	/config conffile [file!]    "default: a+v.conf"
	...
	default conffile: to-red-file %"a+v.conf"
	...
	if error? msg: try/all [
		config: data-store/load-config/name/defaults conffile default-config
		;-- config should not override command line options
		foreach [name value] config [
			all [
				name: in :a+v name
				not get name							;-- override none for values, false for refines
				set name :value
			]
		]
	][
	...
	; print mold config print mold quiet
	...
	if afile [
		if config/last-index: last-index [						;-- remember the selection
			try [data-store/save-config/all/name config conffile]
		]
