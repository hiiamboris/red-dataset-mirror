...
grab: function [
	...
	/mangle				"Save under a short opaque name"
	...
	do [
		...
		if select [
			...
			view/tight/options/flags [
				...
				on-key  [if event/key = #"^[" [unview  aborted: yes]]
				on-down [xy1: xy2: snap shot to pair! event/offset redraw]
				on-up   [shot: crop shot xy1 xy2  unview]
				on-over [if xy1 [xy2: snap shot to pair! event/offset redraw]]
		...
		name: either mangle
			[enbase/base to #{} random/secure 255.255.255.255.255 58]
			[rejoin [%screenshot- dt/year month day '- hour minute second]]
		name: rejoin [dirize dir name %.png]

		save/as name shot 'png
		; maybe-print to-local-file name
		if clip [write-clipboard to-local-file name]
		name
	]
]
