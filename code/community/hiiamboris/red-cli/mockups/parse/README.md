...
Binaries: [Windows](https://link.storjshare.io/raw/jx4mhyld6tltxxfjekouysbhziwa/bin/parse.exe), [Linux](https://link.storjshare.io/raw/jx4mhyld6tltxxfjekouysbhziwa/bin/parse), [Mac 32-bit](https://link.storjshare.io/raw/jx4mhyld6tltxxfjekouysbhziwa/bin/parse-mac)
Compiling: `red -r parse.red`

```
$ parse.exe --help

Parse tool 30-Mar-2025 - Process input using Parse commands - 

Syntax: parse [options] <input> <rule>

Options:
                  <input> File or file mask to parse
                  <rule>  Rule to match against, written in Red PARSE dialect
      --init      <code>  Evaluate given initialization code before parsing
  -l, --lines             Parse input line-by-line (default: as a single
                          string)
  -e, --enum              Display line numbers together with the text (implies
                          /lines)
  -n, --count             Count matches and display the number
  -c, --collect           Collect matches and print to the console (don't
                          forget to use 'keep' in the rule!)
  -w, --write             Write the contents back (incompatible with
                          --collect)
      --whole             Match rule to the whole text (otherwise look for it
                          inside the text)
  -r, --recursive         Look for the file or mask in all subdirectories
...
parse -c -l --whole FILE "0 8 skip keep copy _ 0 8 skip"

   Extract all line comments from the script:
parse -c -l --whole parse.red "to {;} keep to end"
```
...
Printing all lines containing a slash

=== File: D:\devel\red\cli\mockups\parse\README.md
                          /lines)
parse -c --init "mixed?: function [w][s: to string! w not any [s == uppercase copy s  s == lowercase copy s]]" README.md "[any { } copy w word!] opt [if (mixed? w) keep (transcode/one w)]"
16                                        (implies /lines)
48      parse -c FILE "any [thru [any { } copy w word!] (w: to string! w) opt [if (not any [w == uppercase copy w  w == lowercase copy w]) keep (transcode/one w)]]"
