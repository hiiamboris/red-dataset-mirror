...
context [
	...
	dim: func ['p [set-path!] v] [						;-- promotes path to a point2D when one of the components becomes a float
		attempt [return set p :v]
		-p: as path! append/part clear [] p back tail p
		if single? -p [-p: -p/1]
		set -p to point2D! get -p
		set p :v
	]
	...
	dist?: func [xy1 [pair! point2D!] xy2 [pair! point2D!]] [
		xy1: xy1 - xy2
		xy1: xy1 * xy1
		xy1/x + xy1/y ** 0.5
	]
	...
	intersect-boxes: function [g1 [block! object!] g2 [block! object!]] [
		...
		all [
			...
			y1 < y2
			compose [offset: (as-point2D x1 y1) size: (as-point2D x2 - x1 y2 - y1)]
		]
	]
	...
	clip: function [
		...
		either vec/:axis >= 0 [							;-- to the right/bottom
			...
			dim g1/size/:axis: new
		][												;-- to the left/top
			new: g1/size/:axis - nearest/:axis + g1/offset/:axis
			if new <= 0 [return no]						;-- no way to sanely clip it, so don't
			dim g1/size/:axis:   new
			dim g1/offset/:axis: nearest/:axis
		]
	...
	fill: function [
		...
		unless bad? [
			...
			foreach x [x y] [
				...
				pads: paddings-of fa geometries'
				dim geom/offset/:x: geom/offset/:x - pads/1/:x
				dim geom/size/:x:   geom/size/:x + pads/1/:x + pads/2/:x
				...
				if geom/size/:x > (margin/:x * 2) [				;-- face is still nonzero after subtraction?
					dim geom/size/:x:   geom/size/:x - (margin/:x * 2)
					dim geom/offset/:x: geom/offset/:x + margin/:x
				]
			]
		]
		geom
	...
	handle-resize: function [
		...
		foreach fa pa/pane [
			...
			foreach [x anchor] compose [x (x-anchor) y (y-anchor)] [
				if anchor = 'ignore [
					dim offset/:x: fa/offset/:x						;-- use current geometry, not the one remembered
					dim size/:x:   fa/size/:x
					continue
				]
				scale: 1.0 * pa/size/:x / max 1 area/:x				;-- scale everything compared to the initial size
				dim origin/:x: origin/:x * scale
				dim size/:x:   either anchor = 'fix [size/:x] [size/:x * scale]
				dim offset/:x: origin/:x * (pa/size/:x - size/:x) / max 1 pa/size/:x
		...
		forall to-fill [
			...
			either find/match to-fill [y x] [			;-- clip/expand over both axes at once
				...
				dim geom'/offset/:x: new'/offset/:x
				dim geom'/size/:x:   new'/size/:x
			]
		]
	...
	unless find/same system/view/handlers :evt-func [	;-- multiple includes protection
		fix-styles
		insert-event-func 'elastic-resizing :evt-func
	]
]
