...
; change-dir to-red-file %"D:\devel\red\red-src\d2d"
; TODO: (cached) indexing of github data instead of local! (need to think when to update the cache)
; TODO: way to load any file by browsing to it, without actual symbol lookup (e.g. first list would filter by filename not by words, or slash will enter this mode)
; TODO: use fast-lexer's transcode to scan the words!
; TODO: save/load the map if it's fast in fast-lexer - once it stops crashing
...
; TODO: text search (e.g. in strings - for error messages)
...
; TODO: CLI paths to ignore
; TODO: launcher & autorestart with gc off if GC crashes it?
; TODO: some function list panel? may need to rethink the UI then
; TODO: migrate this to spaces
...
rules: context [
	...
	-not-word-1st-:  union -word-prefix- charset "0123465789"
...
system/view/VID/styles/text-column: bind [
	...
	template: [
		...
		actors: [
			...
			on-wheel': func [fa ev] [
				; scroll fa  (pick [up down] ev/picked > 0)  20%		;@@ 20% doesn't work for the touchpad
				scroll fa  (pick [up down] ev/picked > 0)  5%
			]
...
relax-reactivity: function [] [
	...
	set 'reactor! make reactor! [
		...
		on-change*: function spec-of :on-change** [
			unless :old == :new [on-change** word :old :new]
		]
	]
...
load-glob-module: has [e u1 u2] [
	unless value? 'glob [
		print "No GLOB module found, fetching from remote location..."
		u1: https://codeberg.org/hiiamboris/red-common/raw/branch/master/glob.red
...
make-view: does [
	...
	view/tight/flags/options compose [
		title "Red Source Code Explorer"
		columns: panel [
		...
		actors: object [
			...
			on-created: func [f] [
				...
				set-focus columns/pane/1/pane/1
			]
