...
once visuals-ctx: context [
	...
	;-- some gaussian kernels that round to integers
	;-- using `repeat i 255 [print [round/to i ** -1.25 0.001 1 round/to i ** 0.25 0.01 i]]`
	;-- 27 20 18 (11) - too much cutoff, rad=2.22
	;-- 9  4 3 (0.75) -> 81:9 = 9 span, rad=1.34, 3/(3+4)=109/255, 7/(7+9)=112/255
	;-- 10 3 2 (0.26) -> 100:4 = 25 span, rad=1.11
	;-- 24 5 3 (0.22)
	;-- 16 2 1 (0.03)
	;@@ TODO: test this gaussian blur before replacing the old one with it:
	; blur5x5: function [i [image!]] [
	; 	split: func [ofs] [
	; 	    ix: draw ix compose [image i (-1x0 * ofs) image i50 (1x0 * ofs)]
	; 	    iy: draw iy compose [image i (0x-1 * ofs) image i50 (0x1 * ofs)]
	; 	]
	;     join: func [tgt src alpha] [
	;     	src/alpha: alpha
	;     	draw tgt [image src]
	;     ]
	;     i50: copy i
	...
	;     ix: iy: i/size
	;     split 1
	;     i1: join copy ix iy 128
	;     split 2
	;     i2:  join copy ix iy 128
	;     join i1 i2 109
	;     join copy i i1 112
	; ]
