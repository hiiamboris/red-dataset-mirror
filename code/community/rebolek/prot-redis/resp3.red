...
p: context [
	...
	types: [
		simple-string	[to string! value]
		simple-error	[make error! to string! value]
		blob-string		[to string! value]
	]

	keep: func [] [
		print [#keep type value]
	...
	set 'parse-resp func [data] [
		output: make block! 10
print [#-- #parser mold to string! data]
		parse data [
			some [
				!aggregate-type (keep)
			|	!simple-type (keep)
...
;		(keep [#"+" form value crlf])
		(value: form value)
		(keep [#"$" length? value crlf value crlf])
		; NOTE: lossy conversion that turns `a:` into `"a"`
	]
	...
	set 'make-request func [data][
		...
		parse data: copy data [some type]
print count
		insert output rejoin [#"*" count crlf]
		output
	]
]
