...
;	if preview [header/Accept: "application/vnd.github.mercy-preview+json"]
	if preview [header/Accept: "application/vnd.github.squirrel-girl-preview"]
...
get-issues: func [
	...
	ret: send/full/preview link

	either total [
		to integer! first parse ret/headers/link [collect [thru "next" thru "page=" keep to #">"]]
	] [
		ret/data
	]
]
...
get-issue: function [
	...
	repo: form repo
	number: form number
	query: copy [%repos repo %issues number]
	if comments [append query %comments]
	send query
]
