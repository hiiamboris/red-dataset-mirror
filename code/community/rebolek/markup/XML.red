...
; -- options
	...
	default-opts: #[
		...
		indentation: "    "		; indentation spec to use
...
; -- support functions
	...
	set 'load-xml func [
		...
		str-keys?: str-keys
		unless string? data [
			data: either rebol [read/string data] [read data]
		]
	...
	init-charsets: has [word spec high] [
		Char: charset [
			#"^(01)" -#"^(D7FF)" #"^(E000)" - #"^(FFFD)"
		]
		...
		unless REBOL [
			; NOTE: Rebol currently doesn't support codepoints > FFFF
			;       so we use them for Red only
			Char: union Char high: charset load {#"^^(10000)" - #"^^(10FFFF)"}
			NameStartChar: union NameStartChar high
		]
	]
...
; === encoder part ===========================================================
				...
				also collect [
					case [
						...
						any [
							...
							all [not equal? data/1 text-mark string? data/2]
						] [
						...
						equal? data/1 text-mark [keep data/2]
					]
				]
				i-
			]
	...
	set 'to-xml func [
		...
		; process options
		; FIXME: this overwrites options with default one
		;        so to enable trace, XML/default-opts/trace?: true is needed
		;        instead of XML/trace?: true
		set-options default-opts
		fmt: set-format fmt
		if indent [indentation: indent]
		pretty?: pretty
		; do parsing
		copy encoders/:fmt/encode data
	]
