...
;		message: copy message ; NOTE just for testing
		...
		/local act: select types probe mqtt-state/type
		act
		probe mqtt-state
	...
	props-spec: context [
		payload-format-indicator:	none
		message-expiry-interval:	none
		topic-alias:				none
		response-topic:				none
		correlation-data:			none
		user-property:				none
		subscription-identifier:	none
		content-type:				none
	]
...
; to bitset! skip to binary! take msg 3
	...
	print "action PUBLISH"
			...
			length: length - (mqtt-state/taken + prop-length)
			...
			props-ctx: make props-spec [
				user-property: make block! 8
			]

			#TODO "Datatype conversion"
			parse probe props rule: [
				some [
			; ------ payload format indicator
					#{01} copy value skip
					(props-ctx/payload-format-indicator: value)
			; ------ message expiry interval
				|	#{02} copy value 4 skip
					(props-ctx/message-expiry-interval: to integer! value)
			; ------ topic alias
				|	#{23} copy value 2 skip
					(props-ctx/topic-alias: value)
			; ------ response topic
				|	#{08} copy length 2 skip (length: to integer! length)
						copy value length skip
					(props-ctx/response-topic: value)
			; ------ correlation data
				|	#{09} copy length 2 skip (length: to integer! length)
						copy value length skip
					(props-ctx/correlation-data: value)
			; ------ user property
				|	#{26} 2 [
						copy length 2 skip (length: to integer! length)
						copy value length skip
						(append props-ctx/user-property value)
					]
			; ------ subscription identifier
				|	#{0B} var-int-rule ; FIXME: This rule does not exist
					(props-ctx/subscription-identifier: value)
			; ------ content type
				|	#{03} copy length 2 skip (length: to integer! length)
					copy value length skip
					(props-ctx/content-type: value)
				]
			]
