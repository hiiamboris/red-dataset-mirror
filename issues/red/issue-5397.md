...
#5397: [Regression] View module won't compile anymore
================================================================================
Issue is closed, was reported by hiiamboris and has 6 comment(s).
[status.built] [status.tested] [type.bug] [type.compiler]
<https://github.com/red/red/issues/5397>

See https://github.com/red/red/commit/e923891a1881a8c16672a67622768b03d275ee17

**Describe the bug**

Script: `Red [needs: view] view []`
Compiling it gives:
```
-=== Red Compiler 0.6.4 ===- 
...
Compiling libRedRT...
*** Compilation Error: include file not found: styles.red 
*** in file: ??
*** near: [context [
        styles: #include %styles.red 
        extras: #include %backends/windows/styles.red 
        extend styles extras 
        GUI-rules: context [
            active?: yes 
            debug?: no 
            processors: context [
                #include %rules.red #include %backends/windows/rules.red
            ] 
            general: [] 
            OS: [
                Windows [
        ...
        opt-as-integer: function [value [integer! float!]] [
        ...
        resize-child-panels: function [tab [object!]] [
            if block? tab/pane [
                tp-size: tab/size 
        ...
        make-actor: function [obj [object!] name [word!] body spec [block!]] [
            ...
            append obj/actors load append form name #'0000003A 
...
1. Compiled
2. Sane compiler output verbosity on errors

**Platform version**
```
Red 0.6.4 for Windows built 6-Oct-2023/18:46:04+03:00  commit #5120133
```
...
On 2023-10-07T12:45:42Z, dockimbel, commented:
<https://github.com/red/red/issues/5397#issuecomment-1751703853>

    1. Can't reproduce, tried both release and dev compilation modes. Tried with that exact script, tried with other view scripts. All compiled from Red repo, not using `red.exe`.
    2. Fix provided.

--------------------------------------------------------------------------------

On 2023-10-07T12:51:19Z, dockimbel, commented:
<https://github.com/red/red/issues/5397#issuecomment-1751704980>

    Found the culprit!

--------------------------------------------------------------------------------

On 2023-10-07T13:01:20Z, hiiamboris, commented:
<https://github.com/red/red/issues/5397#issuecomment-1751707122>

    Yes, I used binary compiler. `red.r` is fine as it turns out.
