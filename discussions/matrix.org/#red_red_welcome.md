...
R2 dont see COM4 😟 https://ibb.co/Y0HDsG5
...
Welcome @JohnBlood_gitlab hope you like programming & Red 😉
...
@gltewalt Thanks for be hands on! @greggirwin I do chat about the game on [gui-branch](https://gitter.im/red/red/gui-branch) and the code is ready on the link @gltewalt wrote 👌
...
[11:09:22](#RnEpmeJf-YTSOAtxU2M_d0gvJCOBcj2zLTPyjNfjrQQ) <rebolek (Boleslav Březovský)>:
@Sergey_Vladivostok_gitlab I'm afraid it'đ not possible anymore.

[11:09:29](#aeQa0dhpgJYkd-HjBHd5_CVtYOqHxaHA_pcUKtfIAaE) <rebolek (Boleslav Březovský)>:
* @Sergey_Vladivostok_gitlab I'm afraid it's not possible anymore.

[11:09:44](#-HDDfIL2AyUfOZYL1ooWH3GVpVs_yEg5HEgbH5JBEmg) <rebolek (Boleslav Březovský)>:
...
[13:04:03](#NEXr52Q6tvg_uwQfSRdJK_hq83tnfypcgikNufOs1Co) <rebolek (Boleslav Březovský)>:
...
[7:10:40](#pvGsndCOrNNLbWCvBxxcC1WnMB2g5dyecFe9IOEYxaE) <rebolek (Boleslav Březovský)>:
As @pekr  wrote, only Red's compiler is written in Rebol. Red's SSL is not dependent on Rebol in any way.



[7:10:55](#6xR32CdrGR6QcFuv9Ny85YYEjY5FaxK4FGTO15ktlk8) <rebolek (Boleslav Březovský)>:
...
[8:02:58](#_PFdzt-7crflW6YdGerWrx1zjMHTZ75_nwChseXTpmg) <rebolek (Boleslav Březovský)>:
@Sergey_Vladivostok_gitlab I see. The IO branch is fortunately getting very usable so let's hope it will get merged soon.

[8:03:05](#D1qFzpWrLwhsc2IVcYqTyvx2JYiof94F3CKnO4ctuy0) <rebolek (Boleslav Březovský)>:
...
[8:08:19](#hdKD9B9_S5c_OakpObXpLQG_ziBLX9jQ56mH0iRWgnE) <rebolek (Boleslav Březovský)>:
...
Hi everybody, it’s a beginner question but how can you load a csv file in red? Thank you for your help
...
[21:47:27](#mKj82wxPazSsWqOoDuxmS2XedC3GJFSLKdjtbP1wdP8) <ldci (François Jouen)>:
...
@ldci Unfortunately I don't have a Mac machine for testing 😔

[22:59:56](#sB_Hc6UjsLjpHKcwNogCDvW9F84_P98gLEIwOyII8hg) <ldci (François Jouen)>:
@planetsizecpu Don’t worry.If I had time I’ll check for OSX
...
[16:42:44](#nX63e71ktSvh7N2ZxdwRNDch8QHdNxOhndYmF00LaMw) <ldci (François Jouen)>:
@Slownite You’re welcome for developing such tools :)
...
[10:03:07](#y-xlB4B_GsGrUd-su4yUqM6zfgiuqJHqq-ifXiKPJk8) <rebolek (Boleslav Březovský)>:
...
[10:10:14](#wGT-JJxd2YAGOqX6mR8KkoBBktssAldYFySwDtBQAPQ) <rebolek (Boleslav Březovský)>:
...
[10:49:20](#iUfQ-Il2iW1SRyeJEmpM4nc7D7XvrBmqqOAyL26D7Uk) <rebolek (Boleslav Březovský)>:
@Sergey_Vladivostok_gitlab You need to compile it yourself, which is really easy.

[10:49:48](#ajXNwy58nPxXqGsyCdjndBcmukgM30D9AckXuVgqNrM) <rebolek (Boleslav Březovský)>:
...
[10:56:34](#DKCe8O4LOiQaVzEA4w12cJ1S1hw41COnHH5zAYgLy0A) <rebolek (Boleslav Březovský)>:
...
[11:01:10](#tyjSJhb8KxNq6j246slKZYZ6MnVDJWokCk9zJtpg28c) <rebolek (Boleslav Březovský)>:
No problem :-)
...
[9:30:40](#Dj6KpUvnpCeStL99WfwYcV1H6z7xTD9y9Ill3Q-MH5o) <rebolek (Boleslav Březovský)>:
...
[12:08:55](#a9OmdNHL3jIMkdpeBA-nVcuMjKbphQ3Yp5XazeYY4LM) <rebolek (Boleslav Březovský)>:
@mxdpeep welcome :)
...
[17:15:46](#Z2UOs8W9iX6i5rlI8BGFTa8-qvaXGxABvi2IbsEmzsM) <ldci (François Jouen)>:
...
[17:21:35](#b8oZlH_mXHQRVB358wSvkYGUDGhh1Bn7C2DCObfxr_U) <ldci (François Jouen)>:
...
[17:36:50](#j9h07YxTTdR1lV5D4ZwLxMtztDxjNPOedwGiY_vnbA4) <ldci (François Jouen)>:
...
[17:40:10](#aF20ZviaDAjToaZ4QHubH5dZEnJShNX7-VOX2loWoZ8) <ldci (François Jouen)>:
you’re welcome. in redCV we can use float matrices for accurate computation, but in final we need integer for a correct rendering on display
...
[15:44:30](#gg6e91hlsOK3t5ef-YE9IZxOkBUoAtRb4z2WQf_bgLo) <ElCarvoOficial_twitter (El Carvo 🇨🇺)>:
Hi 🙋🏻‍♂️

[15:45:13](#2lJhBP1sveUZLd5qQWIZsZougN0NTV5JCL9epzUFAXs) <rebolek (Boleslav Březovský)>:
Hi!

[15:46:37](#EOXvvjOEgHOGeIUbpSM0vutuJ8tgCw6ucRlFG_sQAQQ) <rebolek (Boleslav Březovský)>:
...
[8:44:10](#_huv2mp3c349E6tEcRYid9GZt3rUVIPko2YBtNA9sEk) <rebolek (Boleslav Březovský)>:
@Sergey_Vladivostok_gitlab I reported it so let’s hope it can get fixed soon.

[8:44:44](#h6AOYlNyEnRsKqbMgtV4y9SNKUNU3mKEB-93yMAQuto) <rebolek (Boleslav Březovský)>:
...
[8:59:51](#ihFB5YLU2hLKG_zy9Kned6Lz32AlTBTJKxiZzA1ubRQ) <rebolek (Boleslav Březovský)>:
...
[9:07:27](#Sg3J4c08Yc0o1w6H5vrAb7n0p60xCSrNfG2-q0RYwx8) <rebolek (Boleslav Březovský)>:
Yes, I’m afraid it won’t be very easy.
...
Write your own dialect. Or declare 'f' with thousands of literal arguments of type 'unset! integer!' 😉
...
@lylgithub2021  ☝ [April 14, 2021 4:41 PM](https://gitter.im/red/red/welcome?at=6076f0f6b6a4714a29c3f1ed) 
...
☝ [April 21, 2021 7:11 PM](https://gitter.im/red/red/welcome?at=60804eaa06e2e024e86961fb) ☝ [April 22, 2021 4:46 AM](https://gitter.im/red/red/welcome?at=6080d57e55d78266a65d0eeb)
...
[5:00:58](#51K-IBtpa9Ie9Zff1YRjhm6OKXh3GrG40ALXomZ_HKQ) <ldci (François Jouen)>:
...
@greggirwin you just made his code very inefficient 😉
...
[19:13:31](#VINaQeYTNvtpyGItoxZm34ryvrVh0yhhV34g1GTSM_k) <rebolek (Boleslav Březovský)>:
#2 would be amazing.
...
Let me guess. You wanted to bloat your exe with random junk to make it big and solid looking? 😃
...
[13:05:50](#ZTByhslnfueZZjaYX3D_6GOaH2C21FfvWKBk4J21OZM) <abdllhygt (Abdullah Yiğiterol)>:
...
/home/hardkorebob/.red/console-2021-6-25-57821: error while loading shared libraries: libgdk_pixbuf-2.0.so.0: cannot open shared object file: No such file or directory
 ...
 ╰─λ find /usr -name libgdk_pixbuf-2.0.so.0
...
[14:00:37](#QmXGz04kecGqFNxncE7IhgmHZ8fMpiY2B5Q5IWy6hFA) <rebolek (Boleslav Březovský)>:
R/S is a dialect of Red.

[14:01:30](#qBPiPH4EUDOpZOcnJeP_fUx0x8ZMrqotLEHBXIeYy3g) <rebolek (Boleslav Březovský)>:
It may seem strange because Red is build in R/S, but you should think of Red as a data format, not a language (it’s also a language but the data format part is really important).
...
first, there's a markdown cheatsheet below and to the right of input line here ☺ --->
...
I think it’s perfectly fine to start with Red :) You may find a lot similarities with lisp but I don’t think you need to learn it first to understand Red. It actually may happen that you will learn both at the same time 
...
@greggirwin it would be great to read some word from @dockimbel as we miss his input since long time, I'm sure he is a lot to tell us. 😉
...
[15:26:32](#KIqGfeClu2xVNCdoml1BeYGwuddA1TR2DpMDrt6nLH0) <rebolek (Boleslav Březovský)>:
...
[5:38:55](#L9AfnspmHCzCYVry2BI3pJoP86mygyEZz0UerjuO_Jw) <rebolek (Boleslav Březovský)>:
...
[5:40:17](#M_ZwI_G3rAEJgaGEaDR77qRMxDOOiCtv6ciS0oyyZvM) <rebolek (Boleslav Březovský)>:
...
[5:41:04](#zHfa4Df1kxwWA2-9XMat-zdj3VkDQ6nYpmKM4PxUSLo) <rebolek (Boleslav Březovský)>:
...
[6:10:04](#FtTIORXnGrqjYFpZ3BUNXDxRPhp5lXLlWIM0OP4Z7XQ) <rebolek (Boleslav Březovský)>:
...
[6:55:26](#LjixIq5KrjWppCCZRV8UL7---oc-PjZH85_sYZSRHh0) <rebolek (Boleslav Březovský)>:
newlines are not special at all, red/rebol is a continuos sequence.

[6:57:35](#8T1wRVawSVgUZvr7qGpnmhQ31mXaq9xZFvMtL-A19yA) <rebolek (Boleslav Březovský)>:
...
[7:58:06](#OWR48PNXXzdFIF4QpjHhL0BWmXmJ2FvwXO0Lx-nUHRw) <rebolek (Boleslav Březovský)>:
@3dc1d3 I hope you will 😉
...
I don't see any context problems in your code. You're trying to convert arbitrary text into integer though, that is an issue 😉
...
Here's two calculators to draw inspiration from (on extreme ends of Red-ness 😉
...
No 😉 Implementation is the easiest part 😉
...
[10:04:16](#uxxIem3ULvYhg8SSy0olncnKXAf9cDUfBb_doQbC6wU) <ldci (François Jouen)>:
...
[5:31:41](#SXzP202koCnLPESCyb6zZ35zH3Isk6Zl2qE5kMtvXHA) <rebolek (Boleslav Březovský)>:
...
[15:19:22](#kFlsLQp9kWshu1FaGZnrxAk35UyyQmCMYfVZQo5IvDM) <ldci (François Jouen)>:
...
you're confusing Red types and R/S types 😉
...
[4:26:55](#xm4ZW8758APPX7xNbNQnoxvP7mHxenRlWwM4dD8O64M) <ldci (François Jouen)>:
@RayMPerry .Some notes and samples about Red/S here: https://github.com/ldci/Red-System/tree/main/code

[4:28:00](#OhRJ2u1S4SBFhOt-9j1FuFW0KNm8RMf_xMluITiae78) <ldci (François Jouen)>:
...
Yeah, but to update the instructions we need to know which Debian version to list 😉
...
roadmap is probably WAY out of date 😃
...
To have a deep Red book we should first finish it's design 😉
...
[7:27:54](#6L9gR8yzlzIkuKEKUAk0mUy1gF4iDbPJoF61A7O01_o) <rebolek (Boleslav Březovský)>:
...
[7:29:32](#i7uknSmaLN0ESUWEN9DI-wN8s2PgKAEBPV4J-F6U6Vg) <rebolek (Boleslav Březovský)>:
...
[11:09:58](#xwQnlKfrlJreGVdI3dFa-agL3riA29pwwBNCtUsAp3M) <rebolek (Boleslav Březovský)>:
There are people that prefer that `body-of` would return a copy of the function’s body, so you won’t be able to modify the function. I believe @Oldes is one of them? You can find arguments for both directions, it’s one of those problems where it’s hard to say who is right. I tend to support modification, but then we need a way to mark blocks as immutable which I hope would come.
...
[14:06:55](#-QYivQvifS9C4zb5AquhRynAhSMlaJFgKXVEyDyoJuQ) <rebolek (Boleslav Březovský)>:
@Oldes of course
...
Hi Slownite, pls disable your AV software 😉
...
Been a while since I have done things with red— had to leave it behind without 64 bit mac support. I was wondering if the Mac M1 Arm chips have changed anything in terms of future releases or it’s actualy still more related to 64 builds and not the architecture.
...
[9:32:06](#PsUB1o-Gdny8SJXgETBKhJugElj0c4knmp7P0MUPXXM) <rebolek (Boleslav Březovský)>:
...
[15:12:11](#WOj_mk6IADGDDuGvqmQdGYU1lij0SXrmxicea3_jHZw) <ldci (François Jouen)>:
...
[15:42:14](#DrJ87Hq5bGu6aWKygKDpviYIVkqanYG4sbPnQnxIiiw) <ldci (François Jouen)>:
...
[17:03:08](#D0jtt_msfyjvvMjCj6GRs2oyLjciQF8PC_y-aSwv-PU) <ldci (François Jouen)>:
...
[9:47:43](#PsFWDYbTz1jUuROWkipOEVTrqlc5sBzwf-ZBa4j0bOQ) <rebolek (Boleslav Březovský)>:
...
[6:41:53](#mXBEk7-MktAajf1jINtQq_EzaCmhP-jttdJVgCuVFUA) <rebolek (Boleslav Březovský)>:
...
[9:06:33](#PeVL3w6ANLBvVM0G5BTQ2w4nr8Xgrnho-2HqF_GBw48) <rebolek (Boleslav Březovský)>:
@duskhorn:matrix.org there’s libRed that you can use

[9:07:47](#eh2qEGxPADug_3mWswdIWxDtzj8WqHB4dN3oKINV9J4) <rebolek (Boleslav Březovský)>:
...
Installers usually don't delete itself 😉
...
@Oldes thanks If you are curious what this installation file is for, I have to say  makeing a installation file. for the old game 😉
...
[3:37:25](#jrVEQQh1fG7dN9s-Y1PnF9j06OaabsT9_F8-Z8GDK3U) <ldci (François Jouen)>:
a suggestion
`blk: ["a" "b" "c"]
str:  ""
...
[3:43:16](#hTwDUGYOxr55vHBerdnpcfG8fwzr9UuXGT0EfVSUzeo) <ldci (François Jouen)>:
Better
`
n: length? blk
str: ""
...
[3:43:45](#AICVz551xK1yN91lDdmiFfQWJ-NFYCnE8hZoRHFpHoA) <ldci (François Jouen)>:
`== "a,b,c »`


[3:45:21](#yhAd3AX3PCfcEr2PwGmaO5vN6fzn1_V2uwVO1DkQXis) <ldci (François Jouen)>:
...
[3:48:53](#c4dKfj4gIyPG_nnPLIEm3FtLcMi_b1Kwu2fNLWaUG9w) <ldci (François Jouen)>:
...
[4:26:48](#6ienpCFVb6I-TvAg_Da09nboCOMMtJtRYyV8aWU_UVY) <ldci (François Jouen)>:
...
[master¹²//firman]% sudo red find-replace.r                                                                                                                                                                                  (l@lat:~/firman/)
[sudo] password for l: 
...
[master¹²//firman]% locate libXaw7.so.7                                                                   (l@lat:~/firman/)
...
32bit system are dying fast... maybe too fast. One may ask, why use tiny Red app, which fits on one floppy disk, when it depends on installing so many libraries. `apt-get-update` downloads 16MB, and the rest needs 332MB (in clean 64bit Docker image) 😕
...
[master¹²//firman]% red -c find-replace.red                                                                           (l@lat:~/firman/)
...
[9:03:31](#ya2MggJyQLORuVjJMey-Of7vj_4b30bHHxiWThozQfE) <rebolek (Boleslav Březovský)>:
...
It is said, that there was even implementation done, but banned by Nenad 😕 https://github.com/red/red/pull/4148 
...
I wonder why there must be so much words about it.. as I said.. if there is `mold` and `form` producing same result, why not to have `form` produce something different and reduce the required code? Never mind.. time to read some book and sleep 😉
...
[8:15:05](#oZNk6V58P7ukQJbGdlsfwL_E9AIHukxICDDQ2qFqU6M) <ldci (François Jouen)>:
...
[21:12:30](#fhM-Vp0iDKoCfNTHutzM5D9eOjBAkJjXnnrnLhyjdTo) <ldci (François Jouen)>:
...
[21:32:25](#zyIUGxEkUaE0LIXMITIiQXm4Q1dR7kM_w1G0K6AN66Q) <ldci (François Jouen)>:
...
[6:49:09](#T_7-i233n1xss4oJAiIkpxocO3V8l_NtXFyA78O1CWY) <ldci (François Jouen)>:
in doc directory
...
[7:46:18](#ib8c_ku6XcrPBFWkO9ky1LgBBNZWcl_DY7qQsWo3kC8) <rebolek (Boleslav Březovský)>:
...
[15:32:39](#JQxbWDN1fpypT66emjAX-R3h-5SV1Q-LjsyjsbRFgRQ) <ldci (François Jouen)>:
...
[15:33:06](#II9jALWgkIrv5k2JLZOaa7F1KU9mLy1N4FnQan3TQ4E) <ldci (François Jouen)>:
`print  bitExtract 140 3 6
print  bitExtract 140 3 4`


[15:33:28](#QlKnrkbOpB1Uf9t-ht_T6gfxRMdG532Nepw1yk30r20) <ldci (François Jouen)>:
OK : we get 4 and 1

[15:34:51](#78sxxh3FdA9z8KPpjXvE4raRJzXdQPbOZNCbqnHRRpc) <ldci (François Jouen)>:
...
[16:26:33](#CeZ-DEKJp7XKmm9qlJMxZxQIuO24VRWQUddzJycGCqQ) <ldci (François Jouen)>:
...
[16:26:54](#TcaRtMLU5j_XUsyP_feWxKqFZEEZZOI-l8LCnDpcrtg) <ldci (François Jouen)>:
...
[6:28:03](#k1FcAMtoXhpEdPaQ14EYq8DOJMT3yNxrwCgUgT1GPS0) <rebolek (Boleslav Březovský)>:
Vim
...
[13:46:04](#rU9Ak1uVrmiNPo_T_MuO6WFAgRgnLnled6PX-_n3TK0) <ldci (François Jouen)>:
...
[13:47:14](#8gVvZHpuE0-wf2WKh-daGXcdtsElJFwa7m_CVR2RNyU) <ldci (François Jouen)>:
A simple way to create nice forms: use zenity inside tour Red code :)

[13:47:58](#SVOZR-TJTaMUUTJ-5s5o2kzz5o7QwLlH3Zz2QwHNnb4) <ldci (François Jouen)>:
* A simple way to create nice forms: use zenity inside your Red code :)

[13:48:32](#egREjUplWMb4kAE4p2P1YSZJeNqrygmzDa_71UwzlHI) <ldci (François Jouen)>:
...
[6:46:25](#mL34rVhgpgEmn5IrOf8VWeruwMcsleZaSA9xVFJMpa0) <JoshieOnNFT_twitter (J🅾️shie)>:
...
R2 will also be helpful when you will want to compile your creations 😉
...
[8:10:11](#jL0pnOOo6p5VNEC7SwTk_vRwAIHqKRAiFxLgAWj1ufo) <rebolek (Boleslav Březovský)>:
...
[8:10:57](#hw5PiFI-07roT81ADY5wtSTT2dH-a5UDX26cAgwflqw) <rebolek (Boleslav Březovský)>:
...
[9:22:25](#Z0uBL1XLFrkCK0_kenBbWlNpJxKLEaeDzC_0B3JFH9M) <ldci (François Jouen)>:
...
☝ [February 3, 2023 4:11 AM](https://gitter.im/red/red/welcome?at=63dc5f5a747c6938e1fcc68d) this is something @dockimbel must look at, to better understand how reactivity performs in practice
...
[7:36:11](#7Cr5e5CNbCy9vw-MUG1U9SFsp3lMGwU4sgvtLmac_XA) <rebolek (Boleslav Březovský)>:
...
OK… (*) For Linux 64-bit distros, you need to install 32-bit supporting libraries.
...
as I said above it is: ☝ [February 5, 2023 5:02 PM](https://gitter.im/red/red/welcome?at=63dfb6e6195b0f648282af3d)
...
OS or driver in R/S is a possibility, like it is in C, but you'll be pioneering it (and seeing that you're a total newbie, it will be a rough ride - be warned 😉
...
@dockimbel are you hiding the precious Red/OS from us? 😉
...
Your account has been flagged. my github wow 🤪 after creating two gist 
...
[9:24:09](#mhGqzJnOfG681bfRGir07dkHXIyLRBOWsE18R33ejyY) <rebolek (Boleslav Březovský)>:
:)
...
[14:31:15](#YVnVZLtuMgNY8SrO6PyUYIhiPYYpPGI0Qn7vqjST-Qw) <rebolek (Boleslav Březovský)>:
`repend` reduces block, `append` does not

[14:32:25](#MyyzuAWsBPVZsxCXcl4c1uySo07UjWJu7Qc2hk1qrC4) <htq2222>:
ok I've got it mixed up. 

[14:33:07](#0f2iYNih-odvHvu8mBXBZqEa_u005orwOxrzcauEWAs) <rebolek (Boleslav Březovský)>:
no problem
...
[14:35:34](#IgwaXTWGaFU7ECoENPzecYpfHqD2OTO7YKiZbHHcntE) <rebolek (Boleslav Březovský)>:
...
[14:39:01](#Z4UpKFDoL5IhyYiW0XIqrC5keOBfANOeYO7ijyBJuz0) <rebolek (Boleslav Březovský)>:
...
[15:13:15](#DYlOgS3uJHog-WthTA-NDiFL9aKMFpl1G_SxfDXTm3A) <rebolek (Boleslav Březovský)>:
...
[15:13:33](#JVN4tQzy_ieOPq3x_6P51I2sEc5s4HtA6Z--IAh34rI) <rebolek (Boleslav Březovský)>:
 * `rejoin` returns string, so you need to use reduce there:
...
[15:14:51](#O08I2DBcPHsWTtT8OCSYKu6jMsc4Gym1nZcNjH5elGE) <rebolek (Boleslav Březovský)>:
...
😲 it's amazing when you know how! I was put off learning Rebol/Red a few years ago but started Red again about two weeks ago. The more I get over the learning pain the more I love it! Your help and example here just deepens my appreciation for the language! Many thanks!

[15:23:36](#lR2j8JlITh0w5M6My1rEoOnMGtYEppSgok3GvO4iaIo) <rebolek (Boleslav Březovský)>:
you're welcome :)

[15:25:38](#-jSBvIKuzzd4zPrTRjrk3aWWcL-CfPx6fDEpJKbREzM) <htq2222>:
I was trying to refactor some bash/python code in our production environment, scanning and replacing text in SQL file with over a million lines against another file containing reference ids. I thought why not prove my refactoring concept in Red first. 😁
...
a lot of unlearning to appreciate Rebol/Red way of doing things 😅

[15:28:05](#T1MBBDGgjmHSkUE5COd-xOFWQ5_SS0hnbO-5k2n6anc) <rebolek (Boleslav Březovský)>:
yeah :D

[15:29:53](#xayjWz0W75yx9teTV4nBDQgHh61Wn4MgSeZ0vAK2_vg) <htq2222>:
Parse is certainly a lot more "natural" than regex. I've come up with regex sentences that scares even die hard Perl programmers 🤣 

[15:31:48](#m_8WJRF4QhyauIoyrFNUI19K29RjCz1u9J95MALG95U) <rebolek (Boleslav Březovský)>:
...
no short cuts for me then - need to learn Parse 😅 
...
this is next level red-foo. Going to take me some time to digest! 😅
...
Parse's power is awesome! being able to run commands inside the parse rule allows the use of direct hash lookup based on the matched criteria. Love it!🤩 
...
[7:16:35](#6yV52Xhleixmhf97-YpxGZHEwW51K3HrkGZIJa6VJ78) <rebolek (Boleslav Březovský)>:
...
(sorry, my initial reply was written on mobile, I missed that it was asked in this thread) Implement an sqlite? I don‘t think I understand the question already, can you explain what you want to do? You‘ll need an sqlite database, an sql odbc driver and a odbc datasource configured in 32bit odbc admin panel or a connection string to establish a connection. The odbc schemed itself requires compilation.
...
What a great way to totally confuse the reader! 🙈
...
[7:47:45](#ibpiHO0-kH-XNC6sWpB_vgIsHMo9AbWerq3yS70Ho3Y) <rebolek (Boleslav Březovský)>:
hiiamboris thanks :)
...
rebolek (Boleslav Březovský): thanks ; i already use castr for my aws sdk project
I will search inside ; very good library !

[8:01:07](#ioH2Hfymqz9Cgs8QG4z4v6QRzbt7ozK-6mUuVJxLaF4) <rebolek (Boleslav Březovský)>:
thanks :)
...
Rebol2 IS right
The checksum generated by red contrains a lot of zéro, that's suspicious
...
[9:09:00](#QjDw_lvzCGySZ3gba_Ecvn0vQwkZATDWoMogf7gOBgY) <rebolek (Boleslav Březovský)>:
Interesting.
...
[18:00:14](#TfPWuHyrqdpdHg0CW7dEUGCeVXwjWa8lAJD74ihufLw) <rebolek (Boleslav Březovský)>:
Benoit de Valois: what are you confused about?

[18:01:42](#CJ8PMhwKwSOJCyQfYuG5LNqwCXiAy4TJAlNtI9-aiqQ) <rebolek (Boleslav Březovský)>:
...
[18:03:11](#-qKMXItrWru3R8YqwJ2c76JlB8-QkbSlEXzCWFiy38k) <rebolek (Boleslav Březovský)>:
...
[18:03:40](#ck_HxAVQ327Ge-Qr2OySqA2ncjsGKGkCPGdUNGIp32w) <rebolek (Boleslav Březovský)>:
If you do `write %file.txt "šómé téxť"` in Red, it's saved in UTF-8.

[18:04:31](#bncj5oHP2emToJEBiDdtCmWBOWaTvtvdKwvd6MLyvmA) <rebolek (Boleslav Březovský)>:
Classic ASCII (a-z, no diacritics) is subset of UTF-8, so it's fine.

[18:05:36](#ftZLMEabfd7Kbe4zvfBn6ilJrCyu799dkMSCp4Yt5bA) <rebolek (Boleslav Březovský)>:
...
[18:05:56](#u7EZlBZN7AEY9cRrPMUgbAUior9oMNJatgAdJiJBroU) <rebolek (Boleslav Březovský)>:
...
[18:08:46](#xPW5dMtYjRFAAE6Y9xSRh8ic1sGwnPHnQHOiPTFHUrM) <rebolek (Boleslav Březovský)>:
Here's my script for codepage conversion https://gitlab.com/rebolek/castr/-/blob/master/codepage.red

[18:09:37](#Qagr-RC3pRWxAXC_uP5SwmUJFoWpYdKT0eTil-5nLBQ) <rebolek (Boleslav Březovský)>:
...
[18:13:27](#z1MZBTgzXv8S8w43vB41Vl95p8ZMFagy-Uopj-1TRDQ) <rebolek (Boleslav Březovský)>:
Ah, cool then :) Thanks for using it! 

[18:14:48](#m_LquEs-5Q3mPDTBCiqhzzrro99ORbWqs97bBCla9q8) <Benoit de Valois>:
thanks for doing it :)

[18:18:38](#zGDgBDH8qOOpK8yEOevoQkHT1Ga2ldrOdtKpEH9CZag) <rebolek (Boleslav Březovský)>:
...
[21:00:40](#Hm91WFSLWGbBMs-nGMF9mplVjkVhkWoBeDlGMnfromc) <rebolek (Boleslav Březovský)>:
The easiest way to detect codepage is to get the info from HTTP headers :)

[21:02:42](#B34v9yhdH4Y_DskCMFDH6i8sXfu_Qj12LypxtDHgmys) <rebolek (Boleslav Březovský)>:
...
[13:21:41](#tu1kDY4QlZtQgE_xCMi-4UNipv6u2SL8DmKmEtJhZ-4) <rebolek (Boleslav Březovský)>:
Benoit de Valois: how old is your Red version?
>> checksum/with "data" 'SHA256 "key"                                              == #{
5031FE3D989C6D1537A013FA6E739DA23463FDAEC3B70137D828E36ACE221BD0 
}

[13:22:06](#U4iFJDpcjP2lMmSHKmSr2CKSNeFAwQh51GlGvqZrbHw) <rebolek (Boleslav Březovský)>:
 * Benoit de Valois: how old is your Red version?
...
[13:22:15](#7W-4qTe3FX-XkJ-07mlpzRnCMWHzujgNCFmgwZZdbZI) <rebolek (Boleslav Březovský)>:
 * Benoit de Valois: how old is your Red version?
...
[13:22:38](#cU6wFB5Gh2Luklt9NBu4Lo457NZ0w27Q0SNpGuUczFA) <rebolek (Boleslav Březovský)>:
Ah, it doesn't work with integer only, I see.

[13:23:53](#nD46w_2ZUTuGEQRkYmUwvqn9tz9LouUjZHu0Skc3l90) <rebolek (Boleslav Březovský)>:
...
rebolek (Boleslav Březovský):  Hello ; I use your library (castr) 
...
[7:03:59](#TDs9E64i8trmTit4zSr3QZot2dnG5PSzVGW3S0eF6_I) <rebolek (Boleslav Březovský)>:
See the doc string:
```
...
[7:04:26](#zqW8fXsjqzw71DvtoY2uVeMiiJJTbyi5d06FgyyGpCM) <rebolek (Boleslav Březovský)>:
It's either a `map!` or a `block!` of key/value pairs

[7:05:12](#OukYc_3hO1OyQSyEKF1L6Au1LyFqmoF_1D3-q8ejeYk) <rebolek (Boleslav Březovský)>:
Something like:
```
...
[8:19:39](#Pt3VBnLfmknrpcKuuFZh-oH75xcnsp3fERMLDii9pl4) <rebolek (Boleslav Březovský)>:
...
[8:25:04](#kxmZXehjLE2thMmv-TKuzkUYxDHiszsgdf3bKzYm6BM) <rebolek (Boleslav Březovský)>:
...
[8:25:20](#DzlXtmEcuKpkUGRYTmnv0Bh3T33nhja9Ri84DGnVCwE) <rebolek (Boleslav Březovský)>:
If you can post me your headers privately, I can look into it.

[8:25:44](#Qj1DiJ4YhmDBsfvx-j_QOBX5VIHzlRdcnbK7k_Dzwx4) <rebolek (Boleslav Březovský)>:
...
[8:34:39](#hMZ7yhCJ0drRYT__jQrxEhZ5h2Uj9nKMCqtoi0qgJlQ) <rebolek (Boleslav Březovský)>:
...
rebolek (Boleslav Březovský): first successfull call to AWS API, thanks for castr !
...
[11:26:05](#8nSzCw-9B7vkON6aUtIG33z3tNVvxF8Zihx7wcVbKko) <rebolek (Boleslav Březovský)>:
...
[5:21:15](#B9qi923An5ox3cT2TgkOeBhva5X4f5n5nTs6kX-6fbY) <rebolek (Boleslav Březovský)>:
...
[5:21:34](#BqUhEwvJ7r-caYv8yBonln8dGjJzABHENg3CdJWsXWA) <rebolek (Boleslav Březovský)>:
...
[6:41:29](#oEbbp2pzz5CbyEXaKTTQAIww8CQ2xt3gHsfaZtqdoOk) <rebolek (Boleslav Březovský)>:
...
[7:55:39](#YLvFNrA5zegDn_wN7UBGnF5WW-3LUOUzTXfwiHQ3Cc4) <rebolek (Boleslav Březovský)>:
...
rebolek (Boleslav Březovský): hiiamboris Thank you!
...
[10:40:23](#kGyV8zV8vKX6x1Qj11JlU7fGA-qWrSR4AGknubh0VYA) <rebolek (Boleslav Březovský)>:
...
[18:21:48](#mQascArhwTukqCV5ejPB1CdoNRooMW_7uC7NIVZRkJs) <rebolek (Boleslav Březovský)>:
...
[11:28:36](#0vPko711BCa3-etsCsfk8l29IY0Lp8uqjtT_bzyYkK8) <‎ ‎ ‎ ‎>:
hey guys where can report a bug ?

[12:08:21](#TVIZSPkE6zNl4_WCvteT-tSNlHjvBuSVUNjc-WqInXU) <gurzgri>:
Before opening an new issue, it’s best to post your findings ro red/bugs for others to confirm

[12:08:51](#tFAYMuFvomhRTmKZoeLJDkIHzoe6d4ZaY-IDpLXAJVk) <gurzgri>:
* Before opening an new issue, it’s best to post your findings in red/bugs for others to confirm

[12:09:37](#Npg6jOpjuD45RgyNOFtcvoIA1c9yre4H4jMg5ruLVlk) <gurzgri>:
* Before opening a new issue, it’s best to post your findings in red/bugs for others to confirm

[13:04:13](#LFaXnC3VNlqoaWxRa86_BCae1HeVEKxTf2gqlS3UPhg) <‎ ‎ ‎ ‎>:
...
[14:02:59](#FSoijRjUKWGZzchUgi6rbVHe4p9ApQQi1tA8OveKMNk) <‎ ‎ ‎ ‎>:
thx gurzgri 
...
[4:51:59](#SdC2x1py4G-JhdN6RIvRhupinOQ7sTt4OTOtae5um1U) <ldci (François Jouen)>:
...
[9:07:33](#-up33Xtmj-k4iBEYZNrxcnX2UZa3xbaxJu_kIUrC8ko) <ldci (François Jouen)>:
Thanks :)
...
[15:01:14](#RYTBd0HNlGLkesQBPLqK4U5vaLjIMVAduSfpNZo7w_M) <‎ ‎ ‎ ‎>:
...
[20:21:18](#G6cBRuKkTJecboSAmkq5zn4oouPVQ-X0UFe81vEIOJM) <‎ ‎ ‎ ‎>:
thx greggirwin (Gregg Irwin) endo64  pekr (Petr Krenzelok) 

## 14-Jun-2024

[12:28:30](#1WmTv6V-3Xf4x9GBlceJRYMmDbgsuCxDgduUMf23PZg) <‎ ‎ ‎ ‎>:
...
[13:49:46](#Ha6mXVbpEgx6vuBk_ycymDOhtryBgyDHuMc0l1W2zXw) <ldci (François Jouen)>:
Under GUI
```
;--from GregT
commands: [who ps df]
...
[17:06:20](#q1R54gbCn4SVkGllpHcDYzliiyGhNB7OUCsevMFUhX8) <ldci (François Jouen)>:
...
[14:56:07](#r6TEjxKVZZUQJjgaptdJVWmUfGqICkHNfYMsD5ezlj4) <‎ ‎ ‎ ‎>:
...
in modules/view/view.red, you‘ll likely have to remove that event handler.
...
[13:41:34](#MriG5WzbzRobVPn9_uNG-Mm3tr2qEGbNUO5WF0SBfKk) <‎ ‎ ‎ ‎>:
...
[16:47:49](#GHwjmom0NCTbb0PWrjsa5499BmbV-t2eQZqLrRwgr0w) <‎ ‎ ‎ ‎>:
...
[9:12:05](#IqoEHeHoIIqJyLxzdrVYpICIb5WRSvcU8tdbohy9Vz0) <‎ ‎ ‎ ‎>:
...
[16:19:15](#FTYACH1wV6pe1N3rXBQ6sQJixpgRVkHWjKl4Q77agmA) <‎ ‎ ‎ ‎>:
hey guys how can unhide or enable a hidden or disabled
 button 

[16:19:25](#SRxxTEuOaRJMFnkBMeFZadseE6mLPFG6VG6uUr3a4yw) <‎ ‎ ‎ ‎>:
...
[17:59:15](#j6bPssRi_G1GSnZ-b9rm6g6etLbnZ5AQk50CPVcgJJ0) <‎ ‎ ‎ ‎>:
oh thx hiiamboris 
...
[15:30:09](#zzMBetuihs_Bp6obkU5LzTrw0CQT5YkQHaheB2riX90) <‎ ‎ ‎ ‎>:
...
[15:55:29](#uHz1ScXvIoXlrmbrX5xSjEpwcdUvauiBAQVy9qpL4pI) <‎ ‎ ‎ ‎>:
...
[16:32:27](#3xBnToizQqhWtiLEF7wXwn2WqQ-VPwL_2zj2jLMKqG4) <‎ ‎ ‎ ‎>:
...
[17:17:36](#fM4JZLy9a5q_-BmZc48zcWH3eepfW_r3D3mdYtShwPA) <‎ ‎ ‎ ‎>:
thx hiiamboris 

[18:50:04](#klgQFVEQx2oMWzce_Z30OJ5Trhp-32hBlPCjTtQ_Xjw) <‎ ‎ ‎ ‎>:
...
[21:12:56](#2f9pXFKTzZxigGjE89GN0ACxaw33KHPj7jxdHaMdkQY) <‎ ‎ ‎ ‎>:
hiiamboris:  thx 

## 2-Sep-2024

[9:48:55](#7_rWx4hI66U46k28ZA6F4_HK6ib-dUTPCy55MfD3ztM) <‎ ‎ ‎ ‎>:
...
[13:33:30](#upzASNrFsVSuAv_pzhjj2Q_Ds0kd1eMvhEo9Qj2hj3I) <‎ ‎ ‎ ‎>:
hiiamboris:  oh thx

[15:04:09](#wg1vSiD5Te939z3a3WWJbZNwB8muDUXXGVCYwoBdqCA) <‎ ‎ ‎ ‎>:
hiiamboris: is possible to write Red code in R/S?

[15:10:57](#7TU5xD5foAMxAUk5OXlDByz57pCxx6aqthLt2VyL8EE) <‎ ‎ ‎ ‎>:
 * hiiamboris: is possible to write Red code in R/S?
...
[19:36:23](#OeCF5k5rclyUyRdH5l4rAVG-z9UuLCHyS3PHMmGVQFY) <ldci (François Jouen)>:
Have a look here: https://github.com/ldci/Red-System

## 4-Sep-2024

[17:48:07](#pMZlo4ZkikMwdnEHgGkpMmrSuFQ9n6H83i6d2QiPoac) <‎ ‎ ‎ ‎>:
...
[19:23:57](#t0x4zBs1HxUiq4AYCmNtPb2d1SyVEvIb5oEb7fKIG9o) <ldci (François Jouen)>:
See also https://github.com/ldci/Red-System/blob/main/doc/Programming_with_Red%3ASystem.pdf

[21:05:55](#aslBHocxTEikmh3oBdLTPQxL2wv8R6KIuv9u_l3tq5Q) <‎ ‎ ‎ ‎>:
ok thx hiiamboris  & ldci (François Jouen) 
...
¯\\\_(ツ)\_/¯ you can work around that by adding `a/text: a/text`
...
[2:12:46](#heQxT6m9xbs2NkqHyw3lcYCp9mMVR3_D1AGJW5Vs_0M) <ldci (François Jouen)>:
...
ldci (François Jouen): hiiamboris Thank you so much!

## 9-Oct-2024

[10:27:42](#eyvyFQPoRO4FBUyONaAzTIzd7qIiltqRaO-HdrOMzuY) <ldci (François Jouen)>:
...
[12:26:20](#Jc3eYyj47eknxL6BB8IlVfbCspL-7KNts-23IkOfJn4) <ldci (François Jouen)>:
gurzgri: Oups. I must be dyslexic :)

[12:35:46](#VIeqG7RZWrUR6JQoPIk0FgvzqiKmPqCjcAV7Lybt61o) <ldci (François Jouen)>:
gurzgri: Code corrected. Thanks a lot :)

[12:44:52](#GPdikdUyVlaaWnmWiufnXfwp6BVUwOJte-G2IzpgvlE) <ldci (François Jouen)>:
![](https://gitter.ems.host/_matrix/media/v3/download/gitter.im/e900a86e702901587c037dcf1c968c76c434d46b1843996049216110592)

[17:05:12](#E83g_PAw1fvqymBFM3vOPzohOj_xWu8uEq8pbxckgXc) <ldci (François Jouen)>:
A better image: 

[17:09:37](#9CHggSJc0tTV16hXAOZFngRLG9AfTh-l_FQdu4Q4hlU) <greggirwin (Gregg Irwin)>:
Thanks ldci (François Jouen) . Good stuff.

[17:56:09](#vWey0UHIVSIVOfIPeyndOAVluVFkAsi65kFSYgaq0Sc) <ldci (François Jouen)>:
...
The error message `*** Script Error: event is unset in path event/away?` tells exactly what's the problem. You're function `f` is declared in the global context, in the global context `event` has no value. You'll need to supply `event` as an argument to your `f` function because èvent` is only bound to the `on-over` actors.
...
[16:40:32](#XDbDqApyXx__19eTAphkFWTbr1JdGjFH3aL7tgU5mD4) <ldci (François Jouen)>:
You use not recognized compiler option such as #do. See https://static.red-lang.org/red-system-specs.html 

[20:16:04](#ELGL5h0mDfMl0OKdmd3BK7r-oIlww4t6qMjOKIXHdY0) <ldci (François Jouen)>:
...
[7:00:37](#X_FrAkEqFxAUNcEsvDVFj0vZaxO5JEhE_k8O1Qm438E) <ldci (François Jouen)>:
...
[9:40:05](#sovLF6o9r7iggnIAF5yxEIo1JzVYttkDTbAfME9pO_4) <ldci (François Jouen)>:
Sample. Incl.red :
`Red [
]
f: function [x] [x + 1]
...
[17:29:01](#eznig2ulX8CscBqGDJnRq5ApwiETb-BS2wkrn4li6YA) <ldci (François Jouen)>:
It’s possible move `s: true either s [print f 3][ print f 5]` in your main program
...
[8:40:06](#zpFGwmMFavHGR2Ghk3mElmlnUdiVIZi4j2S9n3ZS68g) <ldci (François Jouen)>:
...
[8:41:06](#P08bc14ZgyqAB4aLEvR-IFAiG0Hd5Jf3qOQm5EPyBP8) <ldci (François Jouen)>:
* As underlined #do is not a Red compiler option but a REBOL pre-processing option. Try this in your main program.
\`
Red \[
\]
...
[13:59:23](#Kg67N6MFHK5mjzEkPtgO1KJp-GanyYucWgxYoW_hVvI) <ldci (François Jouen)>:
No idea :(
...
* In my program, auxiliary functions are gathered in a file(the function `f` in the file of `incl.red` as example). As the program will be distributed by only a `.exe` file, all data and functions the main program(`main.red`) needs has to be included. Some data from external file must be handled by functions in `incl.red` at compile-time（`#do[print f 3`] as example）, and the result is wroten into a file which is then `#include` into the main progam for the reason of program distribution by only a `.exe` file. Then, compile the main program only would get the `.exe` file, and running this `.exe` file needs not any other support files.

[5:23:42](#rupU0Po0dg-Ii3ddZZL_nVQsmvXR1xv0dYq1jYdhJj8) <lylgithub2021 (lylgithub2021)>:
* In my program, auxiliary functions are gathered in a file(the function `f` in the file of `incl.red` as example). As the program will be distributed by only a `.exe` file, all data and functions the main program(`main.red`) needs has to be included. Some data from external file must be handled by functions in `incl.red` at compile-time（`#do[print f 3`\] as example）, and the result is wroten into a file which is then `#include` into the main progam at compile -time for the reason of program distribution by only a `.exe` file. Then, compile the main program only would get the `.exe` file, and running this `.exe` file needs not any other support files.

[5:27:36](#OOs_e0yk9E9RRNd86_7fF1srdMPPR5UCsu0_GvcxVzM) <lylgithub2021 (lylgithub2021)>:
* In my program, auxiliary functions are gathered in a file(the function `f` in the file of `incl.red` as example). As the program will be distributed by only a `.exe` file, all data and functions the main program(`main.red`) needs has to be included. Some data from external file must be handled by functions in `incl.red` at compile-time（`#do[print f 3`\] as example）, and the result is wroten into a file which is then `#include` into the main progam at compile -time for the reason of program distribution by only a `.exe` file. Then, compile the`main.red` only would get the `.exe` file, and running this `.exe` file needs not any other support files.

[5:29:01](#apmWhKEVVZTYGSU3z5qAeVT0QN5RJagLLjpGbJgDSA8) <lylgithub2021 (lylgithub2021)>:
* In my program, auxiliary functions are gathered in a file(the function `f` in the file of `incl.red` as example). As the program will be distributed by only a `.exe` file, all data and functions the main program(`main.red`) needs has to be included. Some data from external file must be handled by functions in `incl.red` at compile-time（`#do[print f 3`\] as example）, and the result is wroten into a file which is then `#include` into the main progam at compile -time too for the reason of program distribution by only a `.exe` file. Then, compile the`main.red` only would get the `.exe` file, and running this `.exe` file will need not any other support files.
