...
[10:38:39](#ZyVxPfObvnXEmy_sar1v8ooL4NCuxhhU_q0noZD6Bow) <loziniak (Maciej Łoziński)>:
...
Also, the combinations `<>` and `<…​>` with any characters in between the `<` and `>` character are not allowed anywhere in the sequence.
```
...
[14:26:24](#E_V8Yplr8wAAEOWCIXHwcW82ALm6Bjw2CcdgIF9Mams) <rebolek (Boleslav Březovský)>:
...
[14:26:55](#Matq6tTCty9D2WLXTdp_QXy1NFxAw-g06GXc8Y923Ek) <rebolek (Boleslav Březovský)>:
...
[14:27:39](#6dYNXOhQTViYUetKeUVhnopp9__BK4yx9XxFK39_YxA) <rebolek (Boleslav Březovský)>:
@Oldes Oh sorry, I read it as "I think"

[14:28:21](#M4Tb8ssYGmaVy4h1zT03o_yAfhT7C4e8HChF1dymjac) <rebolek (Boleslav Březovský)>:
...
/(code)/(code) could be the road to this but how to make it without disturbing refineents 😉 
...
Not mentioning that someone must also write a compiler to support this theoretical code patern. 😕
...
@toomasv This one `//: make op! 😛ath` is the way to start with.
...
[12:32:47](#VzbAVepOW0iU1hT6O_7LsXyl9AXLXBS0CwK3CVWE5tY) <mxdpeep (Filip Oščádal)>:
> There is also asciidoctor which spits out htm from asciidoc by default.
> 
> https://asciidoctor.org
...
@gltewalt we use asciidoctor since always.. and we also do SPA and PWA if that's the new buzz words.. 😃 
...
[15:16:49](#ckgpT7gbsF0ZGcvZLQh8HN5Py2xK4g_MHzKSTa6bhKg) <rebolek (Boleslav Březovský)>:
just add `-t Linux` when compiling

[15:17:11](#ggY9Pdr62IKbuRXoaGu7N50JzsQ64K5Nrbh6XPP15lw) <rebolek (Boleslav Březovský)>:
`-t` means `target`
...
[12:30:41](#WUHO6ec2ClYUh3v--hzhu05rGQOmV-k5FjN8dBgJE6Y) <loziniak (Maciej Łoziński)>:
...
[12:31:02](#MIOR2FBOiBuHyFdVHxaB1FyyaR8sm1i-lhMhos4le-E) <loziniak (Maciej Łoziński)>:
...
[21:58:26](#DGakfjUO4rQBvSSRdckxMqQxl_sWjvFw936q8lBwgOw) <loziniak (Maciej Łoziński)>:
At the time of transitioning from Arie and Mike you [wrote](https://gitter.im/red/docs?at=5e5303dcdafa3029f64ca385) about @x8x .

[21:58:58](#JUaeXgHE-OXQQcU2jklpgVIPoikW5bZ8P7oBDH3NOXg) <loziniak (Maciej Łoziński)>:
* At the time of transitioning from Arie and Mike you [↑wrote↑](https://gitter.im/red/docs?at=5e5303dcdafa3029f64ca385) about @x8x .
...
[14:42:51](#FvrUVTcg3NzMhFf_JPpLpz88NMNPpLX1Izaoj9ywkw8) <loziniak (Maciej Łoziński)>:
...
[14:52:00](#o7pqBYzb1rJ5PShRzQX0qytTtzW4jiycqAXxYpWdFhA) <loziniak (Maciej Łoziński)>:
...
[14:54:52](#uHM4dm6kfu3RPqG1tBEx3Momi9eKJnUrPZgHqRBrX3A) <loziniak (Maciej Łoziński)>:
...
- #red "related" in cli docs
	...
	- For words with less than 4 (back)links, they will be supplemented **randomly** with links to each other. It can be explained by them having one thing in common – be **unrelated** to any other word.
...
[14:55:09](#xsJiuvXB-zSv38Ry-OE77kCx-f3jh3phYf8DCComj0c) <loziniak (Maciej Łoziński)>:
 * Today morning I sat and outlined a simple sketch of rules for this:
...
[14:56:08](#NvRVUC2Dv8Di9zNh7beHSaHbkLOQ7uFSnXyJHBCmeDE) <loziniak (Maciej Łoziński)>:
 * Today morning I sat and outlined a simple sketch of rules for this:
...
[14:58:56](#FIWPq9mt-UCcgbCOMJcjKm4q6OWzY9asC3qkZQ4Cpoo) <loziniak (Maciej Łoziński)>:
...
[14:59:13](#tuR1PBEXUpRxk08GniZx3N-EeTWeqAAAG7S6DFaNUHk) <loziniak (Maciej Łoziński)>:
 * I recall, that some time ago I had an idea about adding "see also" references to `help-string` output, and probably talked about it on Gitter. Does anybody remember the outcome of this discussion? I remember at least greggirwin (Gregg Irwin) had an opinion on that. I searched wiki and REPs, and could not find any record of that.
I'd like to work on that and would like to know what chances are, that this work would be useful and get merged into codebase.

[15:02:45](#uMsp07_g5lNSh291NcxmwJnPJxmBRtGVWVcXfeH02W8) <hiiamboris>:
loziniak (Maciej Łoziński) why this structure? why not just list a block of related words per each word?

[15:07:26](#rgU7PKYAruEGvZCSehi2MsQbrAB6MdsfrI5WPvm-KB8) <loziniak (Maciej Łoziński)>:
...
[15:12:27](#WLAL1BDHh4PwAxqHNKDpy5Ihljb_CuI8XhsPqbSdtKs) <loziniak (Maciej Łoziński)>:
...
[15:14:15](#xYm2lbDdL5Qjv1RAPBKCMxaklPY14F1A4Gu_99bfWD0) <loziniak (Maciej Łoziński)>:
as much as possible and sensible :-)

[15:23:03](#aSBPkkyuysTsItnYH2Gr99krm7f6b9-pewWiXT9Hdlg) <loziniak (Maciej Łoziński)>:
> <@hiiamboris:tchncs.de> loziniak (Maciej Łoziński) why this structure? why not just list a block of related words per each word?

List of related words would be a final result, but when creating it, we could use some rules.

[15:26:12](#8U-mu-ZeE-AdKDXgO7AsaRk_UW4VZen_WcIdkxEEFV0) <loziniak (Maciej Łoziński)>:
 * I recall, that some time ago I had an idea about adding "see also" references to `help-string` output, and probably talked about it on Gitter. Does anybody remember the outcome of this discussion? I remember at least greggirwin (Gregg Irwin) had an opinion on that. I searched wiki and REPs, and could not find any record of that.
I'd like to work on that and would like to know what chances are, that this work would be **useful** and get **merged** into codebase.
...
[15:30:12](#i4fGoRn8Z91z03lF4O7PUyGUCpq93UFT3sbuc5PXqw8) <loziniak (Maciej Łoziński)>:
...
[17:24:07](#AZW_JcHcWywXurMtfrF1OGNcyDtgPvHGvGgk0AaI-ro) <loziniak (Maciej Łoziński)>:
...
[16:34:22](#vmbnTViWDQd8rQcPCtteMPz9E1iZwqCIUi1iRN9FZUE) <loziniak (Maciej Łoziński)>:
...
[16:35:19](#6OZB-QKthtuK__q9uegxcVdYHyTKx7G-_rCksibxPFs) <loziniak (Maciej Łoziński)>:
...
[16:42:55](#D-WOrDORJI7Sum8gLTnuaMTp-yRv04L7w-yo3hEy5PE) <loziniak (Maciej Łoziński)>:
...
[9:08:15](#sbu_IiojKuQV-ts1-if47j0BPlMltZVW9lB8SE2K1M8) <loziniak (Maciej Łoziński)>:
...
[9:08:39](#BCHDpase08OS5O8fNdYtEbUChYWwjvDVc-xc6Dr6pEc) <loziniak (Maciej Łoziński)>:
...
[10:02:32](#XM-1S4L8N0HBVMrAIj0kRFv-thsg_VvqXZ52Ru5Kayw) <loziniak (Maciej Łoziński)>:
...
[19:32:29](#I7QRhpNfSx3CpEadyvpyApeuWfkX5JWdKSRR-V-RzMk) <loziniak (Maciej Łoziński)>:
...
[19:46:33](#GxsqPiFpNOpoWDhG8kPi7EvwF57zAx5l19kx2KKXbC0) <loziniak (Maciej Łoziński)>:
...
[19:47:43](#4qpnGk6EcKv2mfGSn4KS_0q3I-32eLkkr7p3k2HSny0) <loziniak (Maciej Łoziński)>:
...
[20:04:00](#o3F8PLUbFdU6B8bhAAJo5Cq4krXIji17e4eAnghEfmI) <loziniak (Maciej Łoziński)>:
...
[20:10:30](#os6M167YgYAFeFDWN5iHX12fmyt__k8Tu5G1vT3uMB0) <loziniak (Maciej Łoziński)>:
...
[20:16:00](#brE3naVtd1jOtidhQvGsJBcA-OyJgHwR3YhFa00ohwI) <loziniak (Maciej Łoziński)>:
...
Yes, estimates based on e.g. story points are well established and ubiquitous, but in my opinion, too, it’s a pity that far too less people in the industry are privy to duodecimal time based estimates /s
