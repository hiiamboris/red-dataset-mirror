...
[19:22:52](#Q3EKx9clGKmHt-JWdBccBtV6R6DV22tbWaFvXd7MbJE) <ldci (François Jouen)>:
...
[20:04:25](#B-ZW_12lWDG6aMG1u7mZsi3yXzb4ux_mmmAPe4HUg0I) <rebolek (Boleslav Březovský)>:
...
[10:14:38](#5H2_EdLpI-ZbuzFPRIrHR4tiwBmYA9ho5hTPwgVE-Fo) <rebolek (Boleslav Březovský)>:
...
[19:51:46](#UlwK06kPhCMGJ9xPyNuRyu7TGfB6qJRqCGcqUilcmnc) <ldci (François Jouen)>:
...
[20:21:42](#30TnNdOVVC-Vy2RbMBedYPAsRPtf0_hmCyxD0zEpNQo) <ldci (François Jouen)>:
@pekr Very exciting !

[20:50:53](#OjEDGZla7W3QreF8JX_lkSHyg8fKnUg8gj31-EOSNGU) <rebolek (Boleslav Březovský)>:
...
[20:54:46](#aW9PQtCtGkIOM08ZQ1-ndAoqnlqdzYTzUk2GqsMd6Yw) <rebolek (Boleslav Březovský)>:
...
[20:56:51](#uBqlgnrPUKUCft2EFYZJORtQVNpX6f4MD53Gv5JHUVE) <rebolek (Boleslav Březovský)>:
...
[7:50:55](#UjvSt7Xobv_Lim2MlVVZygHqYgqaQjxgVLqgxe_dc-k) <rebolek (Boleslav Březovský)>:
@ldci if you have any questios, just contact me privately so we don't polute this room

[9:49:37](#YQkGJEeum-myrN03Y4lI0iHyxkHRVq2tjHYNhxB0nr4) <ldci (François Jouen)>:
@rebolek OK
...
There should be `if (not empty? o)` before second alternative in `parse` 😳 
...
Percy the Parse 🐁 - finding his way `to end` of mazes 
...
```
>> parse txt [s: collect some [ahead " =>" 😖 keep to " " 2 skip | #" " s: | skip]]
== ["Hello" "World"]
```
...
```
parse txt [s: collect any [some #" " "=>" e: 😖 keep to #" " :e | #" " s: | skip]]
```
Explained:
```
...
collect any [          ; it doesn't really matter if it is `any` or `some`
  ...
  😖                   ;   go back to last recorded position ...
...
☝ [January 26, 2021 7:09 AM](https://gitter.im/red/parse?at=600fa40bdfdbc1437fb1cbfc)
```
...
[10:10:24](#HCx6cn7r9IR45aV7MZiJ09toSJH1X_AO9YL4bNiPm-s) <rebolek (Boleslav Březovský)>:
...
[10:26:56](#z2C-vzKFKwDTm4wonD-8AZe3U67JgY2aSJsMBe0Davk) <rebolek (Boleslav Březovský)>:
right, that's easier
...
@GalenIvanov Working on your solution I got it 15 chars shorter 😄
```
...
@hiiamboris Wohoo! Still 22 shorter than @GalenIvanov 's latest! 👏 
...
This is `very` deliberate and basic, down to the simplest I can make it while  I stumble through it - so... don;t make fun of me 😎 
...
>   some #" " "=>"       ;   we hit some spaces and "=>" (let's call it "separator")
>   e:                   ;   record position after separator
>   😖                   ;   go back to last recorded position before separator ...
>   keep to #" "         ;   ... and keep word between that position and next space
>   :e                   ;   now jump to position after separator and continue from there
>                        ; Second alternative:
> | #" " s:              ;   we meet a space and record position after that, so we remember last position before we hit separator.
...
```
>> level-3-rule: [s: change "==== " {<a href="#Lnk">} copy lnk to newline 😖 thru "#" change "Lnk" (lowercase replace/all copy lnk #" " #"-") to newline insert "</a><p>" newline]
== [s: change "==== " {<a href="#Lnk">} copy lnk to newline 😖 thru "#" change "Lnk" (lowercase re...
...
☝ [February 12, 2021 9:05 PM](https://gitter.im/red/parse?at=6026d19532e01b4f71904f32) What about `to/thru`? Seems it treats integers as expected:
```
...
😄 I'd like to have had that idea!
...
You'll hunt it down eventually 😉
...
☝ [here](https://gitter.im/red/parse?at=603d75a695e23446e4074b3c)
...
😊  I hope it's ok for completed v1 
...
[9:59:35](#AtsvOOv3jNEX_07VIJgy2zY_0QrzYpWh3z3feyap9P0) <rebolek (Boleslav Březovský)>:
`ahead` checks the rule and rewinds. `set`, `copy` and `keep` work as usual. You can argue that they shouldn’t and I probably would agree with you.

[10:00:12](#5An71PuJzp8DDLwfOvssCh6pAjswJix51b4HnHG74fs) <rebolek (Boleslav Březovský)>:
...
[10:02:56](#vTeq_sYHjY2t7FCEJHJ0O_qWR1WRKxfl_Kgdv4m-q64) <rebolek (Boleslav Březovský)>:
Yes, I don’t see a problem there. If you know what to expect there and don’t need to check for validity, then `skip` is good enough (and faster).
...
[10:53:11](#i-XI9YiAwq7B99KaJ_QPn4VIlDBsMCtNWNBxzD_Qh6s) <rebolek (Boleslav Březovský)>:
```
...
[11:06:20](#5nP8WVYLW4FGHPHHiF191xrP6N2MPOGIBUOhWIn7htA) <rebolek (Boleslav Březovský)>:
...
[11:42:14](#GSeeC2Y3ks9zaYHN58g8SqzWeUoSDBevMPKT1NDDfb0) <rebolek (Boleslav Březovský)>:
...
[12:19:17](#6jm8IADWsS1TBXvk9BPhrxVmAQ6Ia76jSPGo9PXABJ4) <rebolek (Boleslav Březovský)>:
Thanks, I take a look. I’m not using `fail` or `reject` very much but `break` is extremely important.
...
@hiiamboris I just FAILed reading the 3478 issue, because my brain REJECTed to understand all of mentioned cases and I must take a BREAK now 😉
...
[14:25:29](#Tj4OhY5oK4Rrdlc5znm61MZzVaj9MyrJ9WtCkmbJ9u8) <rebolek (Boleslav Březovský)>:
BTW, just now I’ve updated my [HTML parser](https://gitlab.com/rebolek/castr/-/blob/master/html.red) so now it can handle even more buggy HTML than before. Stuff like `<b>start<a href="/">inner</p><p.</a>end</b>` and other monstrosities can be parsed without a problem.

[14:26:23](#lgvHA8EuiHg0QBB_GQqRWtOz5sE-J4AgA0K93zZGDKU) <Oldes (@oldes-556aec4015522ed4b3e131da:gitter.im)>:
But `return (true)` is in R3 14x slower than `to end` and longer😕

[14:27:08](#bpq_uZk7G5ajEvcUjPkeUAVFM03j0OoBnaQP4dFSmkE) <rebolek (Boleslav Březovský)>:
14x slower? wow
...
That's an interesting consideration, but probably should be left for a separate REP? 😉
...
[16:49:29](#e_T72OoHS0F-UrEc4qurvV_d83iUyWO_7nCTsph80cg) <rebolek (Boleslav Březovský)>:
...
In Red there's always another way 😃
...
Had to google it 😃 Turns out it's one of the guys who wrote `2001`

[19:16:34](#1g3qkOT98gyDfPypCPFhB2KnDiwqyyGJIlc0HVlTIO8) <hiiamboris (NOT THIS ONE)>:
You know I was staring at a microSD card the other day and thinking.. how could such 1cm thing fit 1TB of data? And what would an *uninformed* person do with it? Chew it? Try to make some tea? 😉
...
Good luck looking for volunteers 😉
...
[21:26:42](#Z0gUgVIG947R9KB6fJk8i0Pk403t0xATnJH-QKXr3Ho) <rebolek (Boleslav Březovský)>:
@greggirwin I believe there’s a function for it in Gritter, I need to take a look.

[21:27:33](#aTnmUYwNGgz3BjhFKvyu7YX1RrshOYf2i9Pyq9anxO0) <rebolek (Boleslav Březovský)>:
...
Did you try it in fresh console? 😉
...
[9:33:30](#AL7qUO1xH0tcn1K4aNoHjU2VWWs4OLGXJfDDq5xl5z8) <rebolek (Boleslav Březovský)>:
...
there's `word:` an `:word` things in there of particular interest 😉
...
@rseger when stuck, parse your code snippet here, someone will offer an advice 😉
...
bah, my `to` hack works but it is painfully slow 😭 why you gotta be right @hiiamboris? 
...
@perk I've implemented your idea of a [parse tool](https://gitlab.com/hiiamboris/red-cli/-/tree/master/mockups/parse) (how I imagined it) so check it out 😉 
Can we improve it?
...
I added `--write` to the Parse tool to batch edit files 😉

[20:27:23](#_RFnnZsi1guebJnvHgBYtCIm0qGlolL-CXGldLPe9jc) <hiiamboris (NOT THIS ONE)>:
With ports it will kill `sed` 😉
...
parse src blk: [
	some [
	 line-rule
	| p: [path! | set-path!] 😛 into [some [defs | skip]]	;-- process macros in paths
	| s: 
...
fair, still stuck in regex land a bit 😳 

[17:14:18](#tQjd00QsQRuMKXmA4YOSGC5ecWV9sMDSWjBFiEOEB04) <rseger (rseger)>:
I just went through a chunk of my code from a week ago. Two things became apparent: I have actually learned a lot, my original cut.. 😭 

[18:15:36](#cBVYFMDd2YT0URdiLd_E1r7Gz_6Jq3MR66Pmwz0XkN8) <hiiamboris (NOT THIS ONE)>:
maybe you can start a "Parse for Regex users" wiki entry 😉
...
== []
>> parse block [[any [s: if (head? s) break | (probe s s: back s) 😖]] :block]
[]
[g]
[f g]
[e f g]
...
parse block rule: [
    (mark: none) 
    s: opt [if (head? s) (done: tail s) 😃one] 
    [
        any [
            s: (done: tail s) ahead needle mark: 😃one 
...
It looks like some people come here not because of Red, but because of Parse alone 😉
...
I see. Do you happen to recall what webpage told you that there's Parse? 😉
...
the image doesn't zoom for me (files.gitter.im inaccessible) 😕
...
[17:11:49](#xM1LqrDbEQ2ygtCGP2gwXRmxDT5vYjNm7tyrltGWPjo) <ldci (François Jouen)>:
...
[17:49:27](#YAoMUJtdZPZm8BC_Uz0-zSRk4fqsLqII3aNuVCCunxI) <ldci (François Jouen)>:
...
[17:51:56](#CJTcsQ77T8s2aHsvKCq27MY3-qPXWdnv8ghiDz-osbE) <ldci (François Jouen)>:
...
[17:59:50](#WP2THumL0tVBcHBVR3b4HmN9A1ZxfAGV3VKvIlsSMgo) <ldci (François Jouen)>:
...
[18:07:31](#l5sbfqR3k68cU6goxlzhc7GO4SYpJmM5raZDktTLNMQ) <ldci (François Jouen)>:
...
Thank you! I hope there will be more to come 😄 

[18:10:27](#IJ5nwBiziDAgjgmNppj74HwnU3rgjSU1KOE6L87xPLc) <ldci (François Jouen)>:
...
Yes, I need to get familiar with redCV and stop reinventing things 😄 

[18:17:46](#QAMzY3a2n3W34gAopwrmaSpTi1vtDMS8w57koboxuYI) <ldci (François Jouen)>:
@GalenIvanov Please feel free to contribute. You’re welcome. 
...
[6:46:58](#dN03upfD-csjCKtVuRrBRZLax3qcE5Ha86GBnK28TRc) <ldci (François Jouen)>:
...
[6:58:49](#r76I-wA23cGr3uZ2GZpivLcQyzvNub4wo7JB_0cMNWY) <ldci (François Jouen)>:
@GalenIvanov I’ll test :)

[8:24:51](#hqBRaiZNmcI2FtpV6zObbOYwOfPr4Ke5nfoMpFS7kn0) <ldci (François Jouen)>:
...
[23:49:10](#s6Y_o-KUJerFfuThzzQWrbvIW2vtvOnJfB30-yLiglM) <ldci (François Jouen)>:
@dsunanda Very elegant!

## 31-May-2021

[0:34:19](#W9ZhfRDU2qP1GJKJWDQpETn4D6OxoZooKahd2ROqtNU) <ldci (François Jouen)>:
I’m really impressed about string processing with Red: powerful and elegant
`trim/with aTxt rejoin [",.;:!?-(){}'" form #"^ » »]`


[0:34:58](#l98AO0HbPWBZZO4i51jZgKvOqyoJ4-ffCDhH2DoQkms) <ldci (François Jouen)>:
Fabulous

[14:39:49](#MWSJaLGfq1aAiNoONEL3giqltuN5m04kXQ1mb1Qqx8M) <abdllhygt (Abdullah Yiğiterol)>:
hi!
which one is logic?
```
...
[14:40:06](#eTmV7tsQZ__o86ei6j3mcctOv5_isLPX1aXj5dkwYbA) <abdllhygt (Abdullah Yiğiterol)>:
```
a: complement charset "<>"
b: ["<" copy a ">"]
```

[15:19:16](#fFmp_SdMBSWepswpCcinOsc3ba-WumSnIq0M6nQPRuE) <rebolek (Boleslav Březovský)>:
second one. if you have charset, you don’t need `to`. But it would be `copy some a`.

[15:31:24](#cpNFlEDYFeRVBssBjnFbQRDQUvi8CfH3L2nTir-sHB0) <abdllhygt (Abdullah Yiğiterol)>:
@rebolek thank you!

[21:21:07](#_HYp4iLdZjqFoMdiBF33xcmjDlEpD1Lv0_y_aCWGXuY) <abdllhygt (Abdullah Yiğiterol)>:
```
any [
            | !satır
            | !işaret
...
[21:21:43](#EwRdD8vLkSaWCE39gQ04vkBrRI2Uic21JW6CwyZVVbo) <abdllhygt (Abdullah Yiğiterol)>:
...
[21:22:20](#qupDu-A5z_SlfAbBLpCmvlS890_DyzPF2sCxS8QJk9s) <abdllhygt (Abdullah Yiğiterol)>:
* or, do i have to add this:
```
script: any [
             !satır
...
[21:22:34](#vXFVCvjn6pnP2uCLB3xwpsJLoZ5BfVaLN-B6FsHDn4A) <abdllhygt (Abdullah Yiğiterol)>:
* ```
any [
            !satır
            | !işaret
...
[21:22:43](#g8QPGV6vbgXPAkLsIzTPeN1M9Y5VTQO3LoNhy6KYudg) <abdllhygt (Abdullah Yiğiterol)>:
* ```
any [
            !satır
            | !işaret
...
[21:23:01](#N-QMtTGxTu85SO9wkc8Yz2merzGwG3WPAgd9_fdUud8) <abdllhygt (Abdullah Yiğiterol)>:
* or, do i have to add this:
```
script: any [
             !satır
...
[22:33:20](#QruqzbLeTGMgQaL6luRHPYiLULaII2zNBGPLi8e1QFg) <abdllhygt (Abdullah Yiğiterol)>:
...
[22:53:33](#Byr9OGkI1yXhYx9mWdzh9LE45BdcrrI0QvfMJibqucg) <abdllhygt (Abdullah Yiğiterol)>:
i see, but i was talking about putting a block into itself

[22:54:54](#oWZ33D9gzeb4vccaaKmcf9OmdfWMlj3ynWbxW33-ax4) <abdllhygt (Abdullah Yiğiterol)>:
```
blok: [1 | 2 | 3 | 4 | block]
```
like a loop
...
[22:55:08](#1v15_L5qU3pKvvPKX6n_uR9GgIJFqjqaTIltiOGTZYY) <abdllhygt (Abdullah Yiğiterol)>:
* ```
blok: [1 | 2 | 3 | 4 | blok]
```
like a loop
...
[20:58:58](#Ag6Z7yTxzoK-JVpVRPSAflji9Y6XDOWQuEnSUjHd-gY) <abdllhygt (Abdullah Yiğiterol)>:
hello!

[20:59:34](#sMlCNLU-lVR8f_sG-Cd1NP-yEXsYMPOXL3QjuySr838) <abdllhygt (Abdullah Yiğiterol)>:
how to use parse/trace? i was using it before, but i couldn't use now.

[20:59:46](#KxRG5dKk0nC2WiOc46sIKJJnEMWd1SUdOhrmfcV_oG0) <abdllhygt (Abdullah Yiğiterol)>:
* how to use `parse/trace`? i was using it before, but i couldn't use now.

[21:02:17](#-CGZRUnzOW0hMGZ1ntSXn_gKI_mpVqTK528VcDvfK7w) <abdllhygt (Abdullah Yiğiterol)>:
ah it is `parse-trace`, i really forgot. long time i don't write ` Red`.

[21:08:42](#LeF5jKKAlHmp-R4Gbxg-EvBpVihXU7BNxuGXmbCt-MQ) <abdllhygt (Abdullah Yiğiterol)>:
...
[18:10:20](#CPxCidYPiZdGUZ9z8e43JQs3FDeHmoP4BzjlJoONq3o) <abdllhygt (Abdullah Yiğiterol)>:
thank you
...
I want to convert "%abc.txt.def.txt" to "%abc.txt.def" — that is to say, only keep the part before the last dot. How can I achieve 
...
[7:17:54](#W0-FdnXE2cIcHzWi-d0JwnpTEjOEFmSjkv5Jk1cZ8GE) <rebolek (Boleslav Březovský)>:
...
[7:20:49](#QJT9Y3-oJb2ze7gUFdEi_C4RoIPuXriT5TwS0tk9ufw) <rebolek (Boleslav Březovský)>:
If you don’t want to use rejoin, `collect`directly into `string!`:
```
out: ""
...
== "%abc.txt.def"
; Interesting :)
>> first parse tail "%abc.txt.def.txt" [collect any [dot keep (copy/part head s s) reject | s: (s: back s) 😖]]
== "%abc.txt.def"
```
...
[6:40:37](#9hIRZ39t7MA2Ix6wJLJWW2niA2nqF78BUsBT7BP7n2s) <rebolek (Boleslav Březovský)>:
...
[6:42:08](#wKXFczGRNfopnRlzbKZrNFtRneYSlw9kGSGHIf4uUcQ) <rebolek (Boleslav Březovský)>:
...
[6:47:59](#hd5HGvLK8bU4PwKkR8FOAmXTT6BI1Bh5-GDDIsrmLBQ) <rebolek (Boleslav Březovský)>:
...
[5:54:45](#blM42y02LbWMHNZKUilqDQYRfMXninJJmy3Wd3ake-Q) <rebolek (Boleslav Březovský)>:
Nice :)
...
[16:25:34](#sjtXwAptOilJtnvgxeYfrU8VkJ3zkcwASgSAHI3fdv8) <rebolek (Boleslav Březovský)>:
...
[10:19:09](#LNxheIWNLcYQ7v279MEboRhdBS78CiDSkn5aSeL2HvE) <rebolek (Boleslav Březovský)>:
First, you are parsing a string, so it doesn’t make sense to put it into block, just use string parsing.

[10:19:50](#UkaBptpDOAChqpLcjijvAMWYAh0yp5XrGdUfg1YAkxE) <rebolek (Boleslav Březovský)>:
Then it’s pretty straightforward.

[10:21:01](#8ypZRB6fgM0DNc9h7SLfM4PE9lBd1dVz5TWgzVXySRo) <rebolek (Boleslav Březovský)>:
```
...
[10:24:46](#JzxfGZ5mUg4-EZPKd2UlRYrTcNJDBb50x0i5SAPPMdw) <rebolek (Boleslav Březovský)>:
...
[10:26:43](#ye3Y65Sl-6zqP0_IUHqTjzBlJsHabnZ5Sq4_Jv9G1Xw) <rebolek (Boleslav Březovský)>:
:+1:
...
[7:21:01](#7trIZKam1ksvoHWVSEOf9BRoj0c9zggyVUFNrKJZn3c) <rebolek (Boleslav Březovský)>:
because they are words

[7:21:53](#X0YbHdV6DZUHMFiJRiUjRpdBrdgpTPCslTHgeqqpbMc) <rebolek (Boleslav Březovský)>:
...
foreach tag-name list [
    	starting?: yes
        only-tag-name: rejoin parse tag-name [ collect [keep "<" s: any [sp (starting?: no) 😖 keep to end | if (starting?) ":" keep to end | skip]]]
        probe only-tag-name
]
{<foo bar="123" baz="2002-02-26" region="En">}
{<foo bar="123" baz="2002-02-26" region="En"/>}
...
Possible reason is, that there would have to be another condition in the code, so it would slow down evaluation 😕
...
My doesn't break strings & comments! ☻
...
[11:36:06](#pWguie5Dfig-hTYNr3GTSGIJcCBVJMu1-g_mwwFVN3Q) <rebolek (Boleslav Březovský)>:
```
>> number: charset "0123456789"
== make bitset! #{000000000000FFC0}
...
[8:33:59](#gwoDnrtwjwFh-X0EjROs8wee8z-jOuciLTebA3Ovecs) <rebolek (Boleslav Březovský)>:
...
[18:43:43](#2d6XA7xR1Zd3niS8c0Upp03GXKO-IPvYHigxyTjuIUc) <rebolek (Boleslav Březovský)>:
Well
...
[18:44:10](#QQxgkobMWiLlFrbIcOFsFP-gNvmvRxlx1HqbOGClXQo) <rebolek (Boleslav Březovský)>:
yes
...
[18:53:13](#tg_n702j0hpSP2_DihWe83DQaLMjSWRS0LdQuKPZVGg) <rebolek (Boleslav Březovský)>:
...
[15:16:32](#cM54JoyIxdDjIp8YsV-ho4jPrOEo9Z36WX35uPU2tws) <rebolek (Boleslav Březovský)>:
Or my favourite: `parse a: ["a" "b" "c"][(rule: reduce [length? a 'string!]) rule]`

[15:17:04](#XhyqlTRa6rQEDOJKChmWwveuoPJsnPQqvI25a7GioP8) <rebolek (Boleslav Březovský)>:
It's longer but it's a general way to make dynamic rules.

[15:19:15](#_TzSouJ57qbMX50RnZJKo7YgpeyQa1v0D_lcNTJMYgc) <rebolek (Boleslav Březovský)>:
...
rebolek (Boleslav Březovský): I don't quite understand what the word `mark` is used for, so how to understand your second solution? Thank you!!
...
[4:04:13](#uKcuFe91LejNlnc47zZzoBFTzgLUL4CxXWPbFXb7fQM) <rebolek (Boleslav Březovský)>:
> <@lylgithub2021-606ac7e76da0373984794a36:gitter.im> rebolek (Boleslav Březovský): I don't quite understand what the word `mark` is used for, so how to understand your second solution? Thank you!!
...
[7:50:55](#dbd3fNmvIJ4pq0bDfLULbY5jV3qgJ8YSD0XI2z_GRiE) <rebolek (Boleslav Březovský)>:
```
...
[7:51:31](#doJn_ZjOCRyrGA1A08jO9AuNFSeji2mJJf3JnxLQl7k) <rebolek (Boleslav Březovský)>:
...
[18:14:52](#Q_h7aEkhVxbOc9S55vOMztzohhgBT2tPtZ-wlFy8d9Q) <rebolek (Boleslav Březovský)>:
...
[18:19:35](#wq4nwMzUQTujq9N5Ag_b6OF8hhL7kt6q5TSEOkYAjXw) <rebolek (Boleslav Březovský)>:
...
rebolek (Boleslav Březovský): greggirwin (Gregg Irwin) Many thanks for your proposal!!
...
[7:19:59](#J5NEVWIkDXShyLioI19cuyONOrEKSOLj1I3xkHk7zW4) <rebolek (Boleslav Březovský)>:
...
[7:56:55](#xzZLpUwnSE3ZrqJu654PAvnuw83zMyV7YCAeSBggKDk) <rebolek (Boleslav Březovský)>:
yup
...
[8:00:33](#hCJXrS43xvRgnDHuAqjCZFlTCA72SIVVsp4U7LH5NcU) <rebolek (Boleslav Březovský)>:
...
[9:32:35](#kjpx8E5s7pJi6NXxkKXpFpGJe-iOaXP4ZyNgEfUtouM) <rebolek (Boleslav Březovský)>:
...
[6:39:29](#1lBm2NwcKfsPxIEY8puCQ4vDdXMAV7LbNjGpPcQl4yA) <rebolek (Boleslav Březovský)>:
...
[8:36:24](#NfgxCgX2t7RQayOUHbjC8EQsSixJdSYUq899eGYkvf8) <rebolek (Boleslav Březovský)>:
...
[8:56:42](#_aOT3g8ngX7bvdEuLXOelC28bfZw2t70zGj2qKYgSPE) <rebolek (Boleslav Březovský)>:
...
[8:58:26](#yMqf7K_Sa9AsoLBt_8TLdUuSU-OTFov5WCHopOS9iHc) <rebolek (Boleslav Březovský)>:
...
[9:07:06](#QRo-cq_GM5W5WANleQS-70atgCtm3qyfF6fPFasHDUQ) <rebolek (Boleslav Březovský)>:
...
[9:09:06](#UGLdkjSHbPZftVl8RIYecFAjkxn_S3O8ZFn_jspCZks) <rebolek (Boleslav Březovský)>:
...
1.0x (2μs)         | 6      | 0      | 0      | 0           | [parse "aaa<bbbb>ccc" [to "<" set x thru ">"]]
4.87x (7μs)        | 6      | 1      | 0      | 8           | [x: parse "aaa<bbbb>ccc" [to "<" return thru ">"]]
-------------------------------------------------------------------------------------------------------------------
```
...
4.02x (2μs)        | 6      | 1      | 0      | 8           | [x: parse "aaa<bbbb>ccc" [to "<" return thru ">"]]
------------------------------------------------------------------------------------------------------------------------
```
...
```
﻿p-result: parse [ opt1 mandatory opt2 ]  [
	any [
...
`ahead` parses the current position but doesn‘t "reposition" on success: "Preemptively matches a given rule".
...
but I'm listening to you, don't I ? ;) even though in a scientist role I would exclude you from statistics as a wicked outlier 😈

[14:21:00](#myp3F1YnBCpNgdo3W3OD0ddmxeeKNlqq_NMXoz_n5W4) <GiuseppeChillemi (GiuseppeChillemi)>:
> <@hiiamboris:tchncs.de> but I'm listening to you, don't I ? ;) even though in a scientist role I would exclude you from statistics as a wicked outlier 😈
...
"So AHEAD on success modifies the working of ANY" - No, just no. 🫨 I think I give up, I have no idea where that comes from and I think I don't want to know. 😉 

Why do even reason about `any` and complicate things further with, well, your ... made up theories ... before understanding just `ahead`. Why always try to be `ahead` of yourself (Cambridge Dictionary: "to do something too early, or before you are ready or prepared")? 🥺
...
"Who does not know the past cannot understand the present and cannot shape the future." — Helmut Kohl 1995
...
[12:21:35](#lj5hMWseyxsg0LVPtiSD5BjUIgOopgG2UX87wOThXcw) <rebolek (Boleslav Březovský)>:
...
beware, there's no turning back to other langs at some point 😅
...
hiiamboris: don't agree with him or I will delete your repositories and backups! 😑

[12:48:39](#ognOB2GwA5vMe232ILknTSFYSICb3MCocCMFgVec-7E) <ldci (François Jouen)>:
...
Aha, I will try. 'I'll be back' If I can't get it working. The reason I want this as a showoff of Red to my son. He is very smart. Studying mathematics and physics on a university in holland. Maybe I can convince him with this code to switch from Python to Red. Then we have another Reducer😀
...
```rebol
Time               | Evals  | S.made | S.expa | Memory      | Code
1.0x (6μs)         | 44     | 1      | 0      | 160         | [parse out: copy text [any [ahead ["Hillary" whitespace "Clinton"] change "Hillary" "Secretary" | to #"H"]]]
1.08x (7μs)        | 32     | 1      | 0      | 160         | [parse out: copy text [any [to "Hillary" ahead ["Hillary" whitespace "Clinton"] change "Hillary" "Secretary" | to end]]]
3.18x (20μs)       | 527    | 1      | 0      | 160         | [parse out: copy text [any [s: "Hillary" whitespace "Clinton" :s change "Hillary" "Secretary" | skip]]]
```
...
```rebol
Time               | Evals  | S.made | S.expa | Memory      | Code
1.0x (197μs)       | 6008   | 0      | 0      | 0           | [parse text [any [to #"x" skip] to end]]
...
Dynamic rule creation has never been impossible nor is it impossible now. It‘s just not possible to have "dynamic incomplete rules" and wordy rules instead of blocky rules. Nothing stops you herr if you accept running *around* walls instead of *into* walls ;)

[11:44:20](#AqKdgF9HvaaJxNUEoZ3qAmXL9CvqjPGe2EUZGXKabTI) <gurzgri>:
* Dynamic rule creation has never been impossible nor is it impossible now. It‘s just not possible to have "dynamic incomplete rules" and wordy rules instead of blocky rules. Nothing stops you here if you accept running around walls instead of into walls ;)
...
[14:43:56](#389YdvQiwpLquZXYLSoPaQM3HbP6jEuDyZItaMaJ_5c) <rebolek (Boleslav Březovský)>:
