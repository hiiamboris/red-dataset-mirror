...
I do have a need to subtract and multiply images though 😉
...
I think it's intended but better ask @qtxie 😉
...
ideally we want `collect into` but it's broken on binary 😟
...
Thanks! But.. er.. this doesn't work  😧
...
Ah right! I forgot that `alpha` is complementary to alpha in `argb` 😉
...
😄  Thanks! Code is below. `transparent` is any func from above. For some reason switching off anti-aliasing seems sometimes not to work and there are still half-tones. I added a step to eliminate these but this makes font fatter. The version above was made without this step.
```
...
[6:39:31](#ciWdjekxJNzgcM2oKnvgVSKMCugNwKDnRaB68BMIf7g) <rebolek (Boleslav Březovský)>:
@gltewalt:matrix.org 
> Vscode to gist pasting is giving me issues? Anyone else have this problem?
...
[7:50:55](#IMCmMyCp-0VPrc9eN0eG6NruV883gsB2tdQenVWSBiY) <rebolek (Boleslav Březovský)>:
@GiuseppeChillemi It seems that Github finally killed basic authentication so I need to update Github API to use tokens. So sorry, it’s currently not possible 😃
...
[15:11:23](#sQAWWMkRsBdVpFzTPxGgZY-iLwGozVz7t5S702QZFWY) <rebolek (Boleslav Březovský)>:
@hiiamboris Gitlab or Github?

[15:12:42](#UEzCWWr1QGbscD_bTBVS2lzLj2v3xFR7Eg7J2um7sEQ) <hiiamboris (NOT THIS ONE)>:
Gitlab!

[15:13:36](#_Y_Q5GPhhaYO-ePqw6NIJpP7Txtc37ef-ctXF1354PI) <rebolek (Boleslav Březovský)>:
I was confused by the Microsoft mentions :)

[15:13:53](#0i3oa483S2e9wALhW_LEQ-MUoM6Z0otyZiZaXWNesik) <hiiamboris (NOT THIS ONE)>:
Yeah looks like they host on Azure 😉

[15:14:13](#YL1y1T-C9p25UyWz2YfgosaVModJX9c71XePnP4wkUw) <rebolek (Boleslav Březovský)>:
However I still have Gitlab only in queue and haven’t tried doing their API yet, so sorry, I’m not sure.

[17:21:16](#LZcKG5YhcdL1jlmm9x1iLTDVlEON2SoNRjIBij1I0lI) <gltewalt (Greg T)>:
Why do people use gitlab?

[17:22:33](#8EYEkhDbFjwDE0Jsmgo0wVG_YuXqH7ZQC-RGcEvDTKM) <rebolek (Boleslav Březovský)>:
Gitlab has (had? maybe Github offers then also now) some better free plans, you can install it locally and it isn’t owned by Microsoft :-)
...
never mind... now I have icon with sizes 16, 20, 24, 30, 32, 36, 40, 48, 60, 64, 72, 80, 96, 128 and 256, but still see used only 24 and 48... maybe it's in some cache😕
...
[11:57:28](#GvOuZsFWpaZlBYAkYA9AVFkUDXkiX-MxQ4Yldq0F_QM) <rebolek (Boleslav Březovský)>:
...
[12:51:52](#9I9lQLf3W3h5pRZ6KeWmIFwyQm85CjMMMmay-mqkVBY) <rebolek (Boleslav Březovský)>:
No, I’m happy with my wife :)

[12:52:03](#m44-fanUEogC_ga_FoUYdl2sHLZe_76hNm2_xGCA-_M) <rebolek (Boleslav Březovský)>:
now with prompt! 

[12:52:08](#mIypmgcjaHzxsIO1fFEPuLML2ZGvYlkABd23KO-0FJE) <rebolek (Boleslav Březovský)>:
```
mysql> ping
#{010000000E}
6 00000405 5
...
[13:01:55](#DkH88B8eOiTZn4-JEr8LQwsdF2mfwYgC7I00JFAjs-Y) <rebolek (Boleslav Březovský)>:
@hiiamboris :) That’s actually intended. It’s probably IO bug (caused by closing the client in the read event) but it helps me to stop the docker container without some unnecessary effort.

[13:02:23](#1f2_PGhpah8iLx4rPPE-oOw4oVaLIfPqg0OGOIH3zTg) <hiiamboris (NOT THIS ONE)>:
Haha nice :)

[13:02:35](#O0pXGarP6iwedlLGuxOTZlRQQVhrVxP6Wc9cqqHYFuk) <rebolek (Boleslav Březovský)>:
But now that I have a prompt I probably can get rid of it and stop it from prompt.

[13:03:09](#MTuOE1V8_vKyAQ24kpciryNkGS8FXu5xTlx_rTngXjQ) <rebolek (Boleslav Březovský)>:
And report the bug  😃

[17:43:30](#-iv8-AWJ46ag6BeognJf5eqgfDU99lMLe2HMiS5FCAM) <Respectech (Respectech)>:
@rebolek Awesome!

[20:48:42](#0c1V2KuW7utacMQG0IuvoZGsPFFQ-Y9jiUG1iMR_fOY) <rebolek (Boleslav Březovský)>:
@Respectech thanks!
...
[10:39:54](#ZZ3r-VHcW6VHIHuDaXxAGlI9Uw9MuQ0YoHrtPJMsvek) <rebolek (Boleslav Březovský)>:
I’ve been doing some HTTP request from Red and looking at them with Wireshark and I noticed some funny things:
...
[10:40:05](#aWxpbgxr_xnmz5BVghLr3IMkUwcHQ18x1jfIfnbjqrA) <rebolek (Boleslav Březovský)>:
* I’ve been doing some HTTP requests from Red and looking at them with Wireshark and I noticed some funny things:
...
[10:58:52](#6bKEGFgeL5c9gy-Cl3GW0GwL02M5IxcDB9pK_z6bPyk) <rebolek (Boleslav Březovský)>:
...
[4:38:30](#JxOA5V0AzrJaXn1hsuBmgg9JWpPp32PgFkbYcRDbcnw) <rebolek (Boleslav Březovský)>:
...
[4:44:13](#wEPrfJMDJNS-ezCfCa-mzl4URzfhMyKgh3U-osSGzBc) <rebolek (Boleslav Březovský)>:
the second one. The first one is default set but qtxie :)

[4:44:21](#2SsbpEEwcP6_OhRuv38AyGqeK8aiFOgp6VU_QTtIuJU) <rebolek (Boleslav Březovský)>:
...
Most likely free at some block of time before 4🕓
...
Looks like it is 45 hours of road time if we bypass Boise, and 46 hours if we go through Boise. I'll see if I can sell Rosemary on the idea of an extra hour on the road. 😜
...
[18:15:22](#6WP-0edvvZMcqnm81cLsrb0-dGJxLlSmsNkd935qbh4) <rebolek (Boleslav Březovský)>:
...
I guess your question points so far out of my cache, that I still have no idea what you're asking about ☻
...
I played with idea Oldes presented in /red room ☝ [September 22, 2021 9:11 PM](https://gitter.im/red/red?at=614b71d15b92082de1b754ee):
```
...
[12:26:00](#XfKNlrjX9v2BsQqhD792cNYc8wAvWuz2dayonmb_x7A) <rebolek (Boleslav Březovský)>:
...
[6:11:35](#-0g9vegKrhc8MYFNGpDYWq0T-T-Nmyi3r1sAVIgG2fc) <rebolek (Boleslav Březovský)>:
404
...
☝ [January 9, 2022 5:21 AM](https://gitter.im/red/sandbox?at=61da54aa5dc6213cd4d14362)
...
[12:26:45](#eASbauxkAArfg6hnEowVlfs5YHLqLMO1NhzZZPE8Pv4) <rebolek (Boleslav Březovský)>:
yes, really nice :)
...
Red‏‎ (154 members) Rust‏‎ (852 members)  they turned it into a task
...
hmm it's like 1/100000000th of program's startup time or what? ☻
...
[18:08:54](#qRgmzTfgKmU1hIN25zG1-qwwV6qT9Xf5645YQdMgAGI) <rebolek (Boleslav Březovský)>:
very nice example
...
>> sort/compare [0 45 90 135 180 225 270 315 360] by-func 😖ine
...
Or vice versa. I never can quite recall the sign 😃
...
Yeah, couldn't do otherwise 😉 Too many different design aspects coming in touch with this.
...
Very cool. 👍️ Would be great to see the actual rule, optional or as a tooltip. I know this is an early prototype of course. Nice work.
...
Wow. Super cool. With this, DiaGrammar tech, gltewalt (Greg T) 's Parse Caddy, and more, `parse` tooling has an exciting future. And rebolek (Boleslav Březovský) can check out the CSV issue. 

## 17-Oct-2023

[6:50:08](#YI4-VGSVJjifmNJuREdiUEmqWEFVDGzNi6y7B4BKpnI) <rebolek (Boleslav Březovský)>:
...
Change It's name to RECOPLACE because it is a mix of REduce COmpose and rePLACE but selective 🤣

[20:03:21](#jUhpRb4dAWjhLyp-_YF3woltpNPRCOwpPZ16UNYI8ds) <GiuseppeChillemi (GiuseppeChillemi)>:
* Change Its name to RECOPLACE because it is a mix of REduce COmpose and rePLACE but selective 🤣
...
About contexts, I simply create 2 contexts, one with my custom functions, another with just the word with the series. Why? Because I don't have a function that selectively ùbinds just one word (or more)
...
Hope you see the light at the end of it ☻
...
it seems VID has slipped into the button `b2` code somehow ☻
...
It still misses something: function execution support. Back to work using Apply! 💪🏻
...
Yes, there are a few others in the wild like this. hiiamboris has one, rebolek (Boleslav Březovský) has one, and I did one many years ago.
...
https://codeberg.org/hiiamboris/red-common/src/branch/master/leak-check.red ☻
...
1.0x (1μs)         | 18     | 0      | 0      | 0           | [x: f2 if block? x [if none? x/1 []]]
1.13x (1μs)        | 20     | 0      | 0      | 0           | [x: f3 if map? x [if find x #none []]]
1.15x (2μs)        | 19     | 0      | 0      | 0           | [x: f3 if map? x [if reason: x/#none []]]
1.31x (2μs)        | 14     | 0      | 0      | 0           | [x: f1 if none? x []]
----------------------------------------------------------------------------------------------------------
```
...
-------------------------------------------------------------------------------------------------------
Time               | Evals  | S.made | S.expa | Memory      | Code
1.0x (1μs)         | 14     | 0      | 0      | 0           | [x: f1 if none? x []]
1.06x (1μs)        | 18     | 0      | 0      | 0           | [x: f2 if block? x [if none? x/1 []]]
1.11x (1μs)        | 19     | 0      | 0      | 0           | [x: f3 if map? x [if reason: x/#none []]]
1.19x (1μs)        | 20     | 0      | 0      | 0           | [x: f3 if map? x [if find x #none []]]
-------------------------------------------------------------------------------------------------------
```
...
```
Time               | Evals  | S.made | S.expa | Memory      | Code
1.0x (2μs)         | 12     | 0      | 0      | 0           | [select-from-map m 'b]
7.34x (18μs)       | 8      | 2      | 0      | 832         | [select-from-object o 'b]
```
...
1.46x (1μs)        | 7      | 0      | 0      | 0           | [select-from-object o 'a]
```
...
```
1.0x (9μs)         | 7      | 2      | 0      | 800         | [select-from-object-v2 o 'b]
1.43x (13μs)       | 8      | 2      | 0      | 832         | [select-from-object o 'b]
```

[21:19:04](#-nOKq8kU0s2cO5HpnKuX3UZELdWvXsSwOF-Ux6IDMnQ) <gurzgri>:
Most likely a misinderstanding, I was nowhere near to suggesting to use `try` or `attempt` for Giuseppe’s use case but rather explaining the conceptual similarity of problematic ambigue return values.

[21:19:24](#IaH0ezRkOhfH7xrh3d2B-YkLoNEgDyQop1ouKYx6J1M) <gurzgri>:
* Most likely a misunderstanding, I was nowhere near to suggesting to use try or attempt for Giuseppe’s use case but rather explaining the conceptual similarity of problematic ambigue return values.

[21:20:00](#C8QnBHYUt0I1Itr6CRC8XHE0gCOENKXfoQGc3Gut8kM) <gurzgri>:
* Most likely a misunderstanding, I was nowhere near to suggesting to use try or attempt for Giuseppe’s use case but rather pointing out the conceptual similarity of problematic ambigue return values.
...
```
Time               | Evals  | S.made | S.expa | Memory      | Code
1.0x (1μs)         | 12     | 0      | 0      | 0           | [select-from-map m 'b]
1.3x (1μs)         | 14     | 0      | 0      | 0           | [select-from-object-v3 o 'b]
6.55x (7μs)        | 7      | 2      | 0      | 800         | [select-from-object-v2 o 'b]
7.12x (8μs)        | 8      | 2      | 0      | 832         | [select-from-object o 'b]
``` 
...
[9:17:09](#JqdoXjevYpt9JpZWd3kGiKI_JntAQjV6XkU6cnYlEj8) <rebolek (Boleslav Březovský)>:
...
[10:54:31](#ll0D-21veFe0FQAuIHZNd3F8g1rpScTTCoC56_URhts) <rebolek (Boleslav Březovský)>:
...
Call it conservative. But sticking to hopeless ad-hoc "imdeprovements" like none-containers or other none-on-(a)steroids-ideas lacking even the minimal amount of effort put in language design isn‘t innovative either and for sure isn‘t getting you things done. Especially not if there are available solutions galore to your problem. No offense.
...
== #(none 'no-key)
```
I guess we‘ll need `#(none 'really-no-key-believe-me)` pretty fast 😉
...
Sorry, I don't follow, I fail to understand how „with none-containers you don't have to invent a new datatype“ and „we have zero falsey containers“ go together. 

I promise you won‘t ever get your none-container - not because I am against it, that doesn’t matter. But because no one except you will ever be convinced that it is the right thing to do - I‘m betting the 🍿 on it I‘m watching this discussion with. 😜

[18:49:29](#Ya8-24Dq41gtyAcN0EoKQ1Y4JtMhPn5U80lXAlKebPY) <GiuseppeChillemi (GiuseppeChillemi)>:
Read the whole thread, it has been written. I also have written multiple times the need of side data with custom informations like /SKIP value requested by Boris. This is needed to extend or use existing structure for new uses, enrich them in specific coding areas. A falsey container has this purpose: having extra info in the case NONE Is returned. You can understand the idea with an open mindset instead of a popcorn one. 😞
...
That’s a great philosophical question about the language’s design! Logically, making `error!` values falsy—like `none` and `false`—could make sense in certain cases, particularly when using logical expressions like `ALL` and `ANY`. Here’s why:
...
1. **Errors Are Real Values**  
   ...
   - Treating it as falsy might imply that it’s "empty" rather than "something went wrong."  
...
🤦‍♂️ This whole notion of changing the language for the sake of getting one function right reminds me of the old joke *'how many persons are needed to change a light bulb near the ceiling?'
...
[14:10:20](#DPWO5O5qX0hTGKZF9bVeqeCgQtdxCcTFC1K3guXI5Qg) <rebolek (Boleslav Březovský)>:
...
[14:12:52](#OedAX1nJJghuLRvwvlvzOHAllKJLt0otABKfxiS52As) <rebolek (Boleslav Březovský)>:
...
[14:24:08](#wJ-51eWN3BQ5AaiqebJ1nLvNbnAeOzOW_KWwjMtP0Ic) <rebolek (Boleslav Březovský)>:
...
[14:25:22](#RnsqmtmfAeGuDkq16ZRe6v6NVxHCCdUdG151m-UzNrw) <rebolek (Boleslav Březovský)>:
...
[14:25:41](#yBh3As7sQLBqTuny0wumMXa_YnNlyUkaSUMn9kQo-fE) <rebolek (Boleslav Březovský)>:
...
[14:36:24](#SzlbTAkR8d_qoxGWUH3rVoRXyscQ-ACQQQmcl0z2irk) <rebolek (Boleslav Březovský)>:
...
[14:48:13](#0IXXJJy4Yzsk9sqDDNxXbFXveHPEcKNMau4UNJOzA4A) <rebolek (Boleslav Březovský)>:
...
[7:58:49](#3nXdXdKwmzru9qtKBzoJTpp49PqKH0pNr713sZVTasc) <rebolek (Boleslav Březovský)>:
...
[11:15:55](#_eVXK1_7dqCdQhT_A4wkQXqZPHn090F4gqbif1KqACU) <rebolek (Boleslav Březovský)>:
...
[14:19:58](#bs-NdpVmFn7K7U1pVCSebGMkj434UnBP3pbj3WcOTE8) <rebolek (Boleslav Březovský)>:
...
[14:23:54](#q5-8ZrxQJRIGxpqec_nzXTI3JEnHyhZJQvrtN-GMR5E) <rebolek (Boleslav Březovský)>:
...
[14:27:17](#41u-x2Lj3U-NEvnWtCyaWsYN2LkZp7rAtbnmYcbBWak) <rebolek (Boleslav Březovský)>:
...
[14:28:13](#0LeeLTWWu0j-8C1BDPa6bJsuDjSw7QlmZXKbdjzsQXs) <rebolek (Boleslav Březovský)>:
...
[14:34:32](#eD4v0BRIG20Gjlu3Hv_7iRpN-Nen0_MLI-b7R5w64Cc) <rebolek (Boleslav Březovský)>:
...
[14:38:41](#YxPMWgq0YoXsk7SpO6MUIehiUvA-LPjUcg8az-Z4xIk) <rebolek (Boleslav Březovský)>:
good luck then!

[14:40:42](#mRDWswcRNnztm_eeEPD5fkOPdXTMrn3bpP10mG-3-vw) <rebolek (Boleslav Březovský)>:
...
[14:45:38](#O8mwcPZwDxiAxAAK400jlMN8jA65pu3mKVFXnRt0Z1s) <rebolek (Boleslav Březovský)>:
I mean at low level. 
...
[15:35:41](#LxHHrQL7RMfahsMiyfsWs8czohrj21ohjA1D2192aNw) <rebolek (Boleslav Březovský)>:
yes
...
[15:36:12](#Dqa4OR9IZ6KxSGJRfaytVGwqgCX4GR2rZRTXnk_YaiE) <rebolek (Boleslav Březovský)>:
...
[15:39:53](#dsPpt4TGihSrmu1I-3T5W8qJ9o4RrB5jZgAtJw1dAqA) <rebolek (Boleslav Březovský)>:
...
[15:45:01](#nznuYayJLwopGvLrzvnntpXxZhsxFRVLuf6guQAmM-g) <rebolek (Boleslav Březovský)>:
...
[16:23:28](#2EsX7xbK0qzMqxKp7qJNzZ271LhidZTKQi6deEHWKVs) <rebolek (Boleslav Březovský)>:
...
[16:26:05](#wHWkDDXMbxZokO5Rvwi0tBAox_6EFhZSzRABUf5-ug4) <rebolek (Boleslav Březovský)>:
...
[16:31:01](#8nALevl0kXWA2-p1ZEZUQlQEBK7_TDS3w0JbiDA35XM) <rebolek (Boleslav Březovský)>:
...
[16:34:43](#G77_ayjJIgSsZDJxnKShqwJh9N9jtvxYR4Q_OL4IVWw) <rebolek (Boleslav Březovský)>:
...
[16:38:52](#F8NK1OpPXDccXOdA0rpK_GgBe96BBzRsXsjWKYuv7DU) <rebolek (Boleslav Březovský)>:
...
[16:40:26](#EhFA1YK45GYIPr46hEegnBQpPeJCjDcBe82Yg1rLiVs) <rebolek (Boleslav Březovský)>:
...
[16:49:22](#IBhuU8OaO3Arlvcg_MrEEuhoMAW3U6DMksaM_A3x8PM) <rebolek (Boleslav Březovský)>:
